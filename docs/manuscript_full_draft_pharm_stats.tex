...
\title{Retrospective predictive calibration of oncology trial similarity for Bayesian historical borrowing}
\author{[Author names]}
\date{\today}
...
\begin{enumerate}[leftmargin=*]
  \item Retrospective predictive calibration of oncology trial similarity for Bayesian historical borrowing
  \item From trial similarity to borrowability: an explainable mixture-prior framework for oncology historical evidence
  \item Explainable retrieval and mixture-prior calibration for Bayesian historical borrowing in oncology
  \item True-date retrospective validation of oncology trial retrieval for Bayesian historical borrowing
  \item A no-expert-label evidence package for oncology trial similarity and Bayesian historical borrowing
\end{enumerate}
...
\[
y_i = \text{candidate endpoint response or event count}, \qquad
n_i = \text{candidate endpoint denominator}.
\]
...
\[
p(\theta) =
\lambda_0 \operatorname{Beta}(\theta \mid 1,1)
+ \sum_i \lambda_i \operatorname{Beta}(\theta \mid \alpha_i,\beta_i).
\]
...
\subsection{Validation without expert borrowability labels}

Expert borrowability labels were unavailable. Validation therefore used internal retrospective and simulation evidence: held-out beta-binomial predictive NLL, true-date temporal subset NLL, simulation operating characteristics, paired Stage 1 backend benchmarking, borrowing baseline head-to-head comparisons, and feature ablation and section-weight sensitivity. All results are interpreted as retrospective predictive calibration or simulation evidence. They do not establish expert-level borrowability of individual candidates.

\section{Results}

\subsection{True date metadata supported temporal evaluation}
...
\subsection{Borrowing baseline comparisons showed improved predictive calibration}
...
\subsection{True-date temporal subset validation supported the calibration signal}
...
\subsection{Feature ablation provided a transparent sensitivity check}
...
\begin{enumerate}[leftmargin=*]
  \item Full simulation scenario definitions and operating-characteristics tables.
  \item True date extraction precision and missingness report.
  \item Full paired Stage 1 query-level metrics and bootstrap procedure.
  \item Full borrowing baseline NLL rows.
  \item Feature ablation and SECRET section-weight sensitivity results.
  \item Leakage-control schema and pseudo-query construction details.
  \item Reproducibility commands for all no-expert-label validation workflows.
  \item Torch-enabled full temporal retraining command templates.
  \item Data availability and raw ClinicalTrials.gov extraction limitations.
\end{enumerate}

\section*{Pre-submission checklist}

\begin{itemize}[leftmargin=*]
  \item Add verified references for Bayesian borrowing, power priors, MAP priors, commensurate priors, Trial2Vec, SECRET, SAM-style prior-data conflict, and regulatory guidance.
  \item Run full temporal retraining in a \texttt{torch}-enabled environment.
  \item Add multi-endpoint validation beyond ORR.
  \item Decide whether the target journal requires a structured or unstructured abstract.
  \item Replace placeholder author names and citation notes.
\end{itemize}
