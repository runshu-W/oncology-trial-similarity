...
Trial2Vec 是可选 Stage-1 retrieval backend：在已经构建 Trial2Vec index 后，可以用 Trial2Vec 表示替代 ClinicalBERT-compatible embedding。SECRET 在本 revision 中仍是 future/interface-only 能力，不是已实现 retrieval backend。
...
当前 rule-based summary 预留了 LLM/SECRET-style summary 的接口位置，但 SECRET 在本 revision 中仍是 future/interface-only，不是已经实现的摘要或检索模块。
...
- SECRET：future/interface-only，在本 revision 中尚未实现为 summary 或 retrieval backend。
