...
当前 embedding 是本地 deterministic hashing embedding：
...
- 只能捕捉词面相似。
- 不能理解医学同义词和深层语义。
- 后续应替换为 Trial2Vec、BioClinicalBERT、PubMedBERT、SapBERT 或 OpenAI embedding。
...
但它仍然基于 hashing embedding，容易受到词面重叠影响。例如两个 trial 都包含 rituximab 或 lymphoma，就可能排得比较靠前，但它们的 line of therapy、histology、endpoint 或 design 未必适合 prior borrowing。
...
pipeline 使用 multi-aspect representation，不把 trial 当作一整段文本。

### 22.4 与 SECRET 思路兼容

当前 rule-based summary 可以替换为 LLM/SECRET-style summary，而下游 embedding、rerank 和 report 结构不需要大改。
...
### 23.1 Hashing embedding 仍是 baseline

当前 first-stage retrieval 仍是 hashing embedding，只能捕捉 token overlap。

后续建议替换为：

- Trial2Vec pretrained encoder
- BioClinicalBERT / PubMedBERT
- SapBERT
- OpenAI embedding
...
保持 multi-aspect 架构，但将 hashing embedding 替换为 biomedical/trial embeddings。
