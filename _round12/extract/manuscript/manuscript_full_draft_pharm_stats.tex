...
five-fold-larger batch; between-design comparisons size-standardized by
randomized-boundary tests at a common target).

\noindent\textbf{Results.} In the primary analysis the final design is
not detectably different from the availability-consistent EB reference
and not yet useful ($\Delta$NLL $+0.006$, $95\%$ CI
...
matched pool sizes without the availability restriction, that the cap is
directly measured to reduce in mean (within-query contrast, capped
minus uncapped: $-0.029$; block bootstrap $95\%$ CI
$[-0.053,-0.010]$). On the upper-bound tiers the capped design's
forward and title-screened deltas are significant against the EB
reference ($-0.041$ to $-0.049$, surviving
donor-block resampling; the exploratory five-seed ensemble is as
well), but every capped real-data result is a post hoc
...
size-standardized average power difference over calibrated no borrowing
that is small and positive in every examined split of the validation
batch (randomized-boundary repeated cross-fitting at a common target,
training-half size exact, evaluation-half average sizes $0.0251$ for
both designs: $+0.0025$), rising
to $+0.013$/$+0.040$ under narrower mixings --- a conditional,
batch-specific comparison, not a confirmed population claim. A cell-specific ROC-efficiency
diagnostic --- which conditions on the unknown true drift and is not an
implementable power benefit --- locates an observed local advantage at
small drift of either sign, with asymmetric magnitude ($\approx +0.07$
near zero, reversing beyond $|{\rm drift}| \approx 1$). The wide corpus
...
The direct paired capped-versus-uncapped contrasts grade by tier: on the
primary rows the cap reduced the mean harm of learned borrowing (robust
to leave-one-query- and donor-out checks), on the
forward tier it improved an already favourable tier mean (a tail-driven,
insurance-like effect), and on
the title-screened tiers the direction is inconclusive.
...
exploratory and corpus-specific: size-standardized, the advantage at
this corpus's drift dispersion is about a quarter of a percentage point
of power, an order of magnitude below its value under narrower drift,
and conditional on the simulation batch and estimated mixing. All claims are graded by frame in the text; expert review,
...
size-standardized mixture-average gain in simulation, conditional on
the simulation batch and estimated mixing ($\approx 0.25$
power points under the corpus drift dispersion, an order of magnitude
larger under narrow drift; an observed cell-specific ROC-efficiency
advantage at small drift), and an upper-bound gain
...
otherwise. Within that limit, confirmatory weight is narrow and specific:
the first two forward windows informed development, and every capped
result is a post hoc redesign evaluation, so only the pre-specified
uncapped comparisons on the final window --- examined once, in aggregate,
under pre-registered criteria and multiplicity control --- carry
(limited) confirmatory weight; a true
...
masquerade as a power benefit; equality of the \emph{training-half}
empirical size is enforced exactly by randomized-boundary tests at a
common target, with the evaluation-half average size then
approximately matched (the test
statistics are discrete, so deterministic thresholds can only
approximate a common size; Section~\ref{sec:goldsim-results}), its
split and threshold-estimation sensitivity is
...
availability-consistently trained fold models --- is not detectably
different from the availability-consistent EB reference: pooled $\Delta$
$+0.006$ (bootstrap $95\%$ CI $[-0.005, +0.016]$; strongest-donor block
...
on these decision-time rows the capped version was not detectably
different from the EB reference whereas the uncapped version was harmful.
The contrast is also measured \emph{directly}, as the within-query paired
difference $\mathrm{NLL}_{\mathrm{capped}} -
\mathrm{NLL}_{\mathrm{uncapped}}$ (nonzero exactly where the uncapped
borrowed mass exceeds the cap, $50$ of the $589$ rows): pooled $-0.029$
(ordinary bootstrap $95\%$ CI $[-0.053,-0.009]$; strongest-donor block
bootstrap $[-0.053,-0.010]$), engaged subgroup $-0.143$
($[-0.258,-0.048]$; block $[-0.258,-0.056]$) --- intervals excluding
zero, so the cap \emph{reduced the mean NLL} in this post hoc comparison
--- consistent with, but
not independent
evidence for, the attenuation observed in simulation
(Table~\ref{tab:directpaired} collects the direct contrasts on every
tier); cap sensitivity
$0.3/0.7$ gives $+0.002$ (CI $[-0.005,+0.008]$) and $+0.013$
(CI $[-0.002,+0.028]$). These pooled contrasts are \emph{marginal}
effects over all decision-time rows; the projection alters only the $50$
binding rows, so the same evidence stated \emph{conditionally on
binding} is a mean difference of $-0.340$ with a heavy left tail: median
$-0.030$, IQR $[-0.39,+0.07]$, $5$--$95\%$ quantiles $[-2.06,+0.36]$,
range $[-4.66,+0.44]$, $30$ of $50$ rows negative. The mean reduction is
tail-heavy but not an artifact of single rows: the binding rows'
$10\%$-trimmed mean is $-0.154$ and $5\%$-winsorized mean $-0.273$;
winsorizing all $589$ rows at their $1$st/$99$th percentiles leaves
$-0.017$; and every leave-one-query-out and leave-one-donor-block-out
mean remains negative (both ranges $[-0.030,-0.021]$; deleting the
single largest improvement, $-4.66$, leaves $-0.021$). By window the
reduction concentrates where borrowing engages: $-0.012$ (block CI
$[-0.037,+0.000]$; $2$ binding rows), $+0.004$ ($[-0.003,+0.014]$; $9$),
and $-0.053$ ($[-0.095,-0.020]$; $39$) for w1/w2/w3
(Supplement~S2 tabulates the tail statistics for every tier). Substituting the anachronistically well-trained
canonical models (fit on the full $632/813/1{,}017$ pools) under the same
scoring filters shows no gain (pooled $-0.002$, CI $[-0.015, +0.010]$;
...
not detectably different from the EB reference there (and thus not yet
useful), the uncapped ablation is
actively harmful,
...
monotonically from harm toward the full-pool models' absence of
detectable difference (Section~S2 for the curve). Together these experiments \emph{support} ---
...
\begin{figure}[htbp]
  ...
  (green) shows no detectable difference from the EB reference anywhere;
  the uncapped version (orange) was
  significantly harmful, concentrated in the engaged subgroup; the frozen
  canonical-model control (grey) likewise shows no detectable difference;
  the direct within-query capped-versus-uncapped contrasts are in
  Table~\ref{tab:directpaired}; and the
...
the per-query proxy rule shows no detectable difference (pooled $\Delta$ vs the
...
\begin{figure}[htbp]
  ...
  is better); the two selective variants --- the final design (capped,
  $2.877$) and its uncapped ablation ($2.872$) --- are the only
  design-time priors
...
\begin{figure}[htbp]
  ...
  design-time priors, now including the \emph{final design} (capped
  selective, $2.803$); the uncapped selective ablation ($2.818$) and the
  disease-stratified
  EB reference ($2.815$) follow, the intercept EB reference is at $2.844$
  (dashed line), and the fixed-budget predecessor trails at $2.884$.
  (b)~Per-window paired delta versus intercept EB with
  bootstrap $95\%$ CIs for the final design (green) and its uncapped
  ablation (red); the most recent, best-trained window is significant for
  both (final design $-0.045$, CI $[-0.074,-0.016]$; uncapped $-0.046$,
  CI $[-0.086,-0.008]$).}
  \label{fig:forward}
\end{figure}
...
before any of these numbers were computed) yields: pooled forward $\Delta$ versus EB
...
$[-0.067,-0.018]$; strongest-by-gate $[-0.063,-0.020]$; Supplement~S2).
Whether the cap taxes these favourable tiers is a within-query question,
so Table~\ref{tab:directpaired} reports the direct paired contrast
$\mathrm{NLL}_{\mathrm{capped}} - \mathrm{NLL}_{\mathrm{uncapped}}$ with
ordinary and strongest-donor block bootstrap intervals: on the forward
tier the cap \emph{improved} the paired score ($-0.0148$, ordinary
$[-0.0301,-0.0011]$, block $[-0.0299,-0.0014]$; the projection binds for
$281$ of $775$ queries), while on the two title-screened tiers the point
estimates favoured the capped version (retrained $-0.0147$, block
$[-0.0371,+0.0054]$; frozen $-0.0108$, block $[-0.0260,+0.0029]$) but
the intervals are compatible with both modest improvement and modest
worsening, and no directional conclusion is supported there. The
forward-tier mean conceals insurance-like structure: the \emph{typical}
binding row pays a small tax (binding-row median $+0.021$, IQR
$[-0.06,+0.11]$; $10\%$-trimmed mean $+0.017$, sign-flipped), and the
mean improvement is carried by a minority of large corrections ($5\%$ of
binding rows improve by more than $0.69$; leave-one-query-out and
leave-one-block-out means nonetheless stay negative,
$[-0.015,-0.012]$) --- the well-trained forward models usually lose
slightly to the cap and are occasionally saved from large over-borrowing
errors, the opposite of the primary tier, where the typical binding row
itself improves (median $-0.030$). Restricting
historical mass acts
as regularisation against over-confident allocations out of time;
measured directly, the projection reduced the mean paired NLL on the
primary rows, improved the forward-tier mean through its left tail, and
was directionally
inconclusive on the title-screened tiers. One
...
\centering
\small
\caption{Direct within-query capped-versus-uncapped contrasts
($\Delta_i = \mathrm{NLL}_{\mathrm{capped}\,(c=0.5),i} -
\mathrm{NLL}_{\mathrm{uncapped},i}$; negative favours the cap; post hoc
exploratory redesign comparisons throughout). The cap projection alters
a query's prior only when its uncapped borrowed mass exceeds $0.5$
(``binding'' rows); every other row contributes an exact zero, and all
rows enter the pooled estimates. CIs are ordinary and strongest-donor
block bootstrap ($B = 4{,}000$).}
\label{tab:directpaired}
\setlength{\tabcolsep}{4.5pt}
\begin{tabular}{lrrrl}
\toprule
Analysis & $n$ & Binding & Mean $\Delta$ & $95\%$ CI ordinary / block \\
\midrule
Primary, pooled & 589 & 50 & $-0.0289$ & $[-0.0530,-0.0091]$ / $[-0.0527,-0.0101]$ \\
Primary, engaged subgroup & 119 & 50 & $-0.1428$ & $[-0.2584,-0.0480]$ / $[-0.2581,-0.0556]$ \\
...
\begin{figure}[htbp]
  ...
  and (c)~$50/80/95\%$ prediction-interval coverage, comparing the
  \emph{uncapped} selective
  ...
  Table~\ref{tab:calib}; the final design's own KS is $0.097$ there).
  The uncapped selective prior approaches the EB
...
This subsection is, first, a mechanism analysis of the \emph{uncapped}
selective architecture --- the final design's allocation before the cap
projection, whose learned mass is what varies freely --- and it closes
with the recomputation for the final design itself (post-cap mass,
capped NLL). The selectivity mechanism is directly inspectable, because the learned
...
$\mathrm{NLL}_{\mathrm{EB}} - \mathrm{NLL}_{\mathrm{selective}}$ (both
uncapped in this diagnostic) over the
...
prior shows no detectable difference against EB ($+0.003$, CI $[-0.007,+0.012]$) where
...
yields a similar observed mean benefit ($+0.035$ versus $+0.058$, both
CIs
crossing zero); neither superiority nor equivalence of the two
selection devices is established. The model learned, from features alone, the
...
Recomputed for the \emph{final design} --- actual post-cap mass
$\min(1-\lamz,\, 0.5)$, which creates a $281$-query tie atom at $0.5$,
against the capped NLL --- the triage structure survives: the
average-rank Spearman correlation is $+0.14$ (bootstrap CI
$[+0.06,+0.21]$; $+0.14/+0.15/+0.13$ across the three windows), and the
graded pattern sharpens at the top because the cap removes the
over-borrowing tail. Non-binding rows below their median mass show no
detectable difference against EB ($+0.005$, CI $[-0.004,+0.014]$),
non-binding rows above it carry a significant win ($-0.051$, CI
$[-0.086,-0.016]$), and the $0.5$ atom --- inside which tertile
membership would be arbitrary, so we report it as one group --- is now
itself significant ($-0.074$, CI $[-0.121,-0.025]$); strict tertiles
with stable tie-breaking give $+0.003/-0.052/-0.075$. Selecting the
atom ($n=281$; it overlaps the rule's selection in $169$ queries)
yields a mean benefit of $+0.074$ (CI $[+0.027,+0.120]$)
against $+0.080$ (CI $[+0.040,+0.120]$) for the pre-specified $I \ge 8$
rule ($n=291$) under the same capped scoring --- the two selection
rules yielded similar observed mean benefits; neither superiority nor
equivalence was established. And because the post-cap mass equals
$0.5$ for $36\%$ of forward queries, the final design's triage
evidence supports \emph{coarse risk grouping}, not a reliable
per-query ranking within the capped set. Where this paper
attributes triage value to the final design, it rests on these
recomputed numbers; the uncapped numbers above are the architecture's
mechanism diagnostic.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.72\textwidth]{F12_triage}
  \caption{\textbf{Learned borrowing triage (uncapped-architecture
  mechanism diagnostic).} Uncapped learned borrowed mass
  ...
  $\approx 0.79$ everywhere (dotted line); the final design caps the
  historical mass at $0.5$ at scoring (dashed line; the post-cap
  recomputation is reported in the text).}
  \label{fig:triage}
\end{figure}
...
$+0.0057$ for the final design. \emph{Repeating} the cross-fit under
$200$ random half-splits with a single \emph{deterministic} threshold
per training half --- approximately size-adjusted, as the diagnostics
in Supplement~S2 establish --- centres the final
design's size-adjusted difference near zero: mean $-0.0007$, median
$-0.0009$ (central $95\%$ split-quantile range
$[-0.0014, +0.0039]$; distribution and dissection in Supplement~S2,
Figure~S1). Because the $200$
splits are half-splits of the \emph{same} validation batch, this
split-quantile range characterises split-to-split and
threshold-estimation
variability conditional on that batch: it is a descriptive quantile
range, not a confidence interval for the population difference, and it
does not include between-batch sampling variability. The diagnosis
(Supplement~S2, Figure~S1) attributes both its negative core and its
eight-split positive tail to size-matching granularity of the discrete
no-borrowing statistic, whose typical evaluation-half liberality
($+0.0006$) is of the same order as the effect being estimated: the
deterministic-threshold analysis is \emph{approximately} size-adjusted
at best. The size-standardized analysis therefore uses the classical
randomized construction: on each training half, for each design,
the test rejects above a critical value $c$ and with probability
$\gamma$ \emph{at} $c$, with $(c, \gamma)$ solving the weighted
\emph{training-half} empirical size exactly; $(c, \gamma)$ is then
fixed and applied to
the evaluation half in expectation (the convex combination of the two
adjacent achievable tests, adding no Monte Carlo randomization noise),
at a \emph{common} target for both designs under every mixing.
Matching on the evaluation half is approximate but tight: the
per-split evaluation-half size gap between the designs has mean
$-7\times 10^{-6}$, central $95\%$ range $[-0.000215, +0.000207]$, and
maximum magnitude $0.00058$ (Supplement~S2). Under the corpus
mixing at target $0.02513$ the size-standardized difference is $+0.0025$
(median $+0.0025$; split SD $0.0004$; central $95\%$ split-quantile
range $[+0.0018, +0.0032]$; positive in all $200$ examined splits; both
designs' evaluation-half sizes average $0.02514$), and the same at
target $0.025$ ($+0.0025$) --- consistent with the full-batch
randomized plug-in ($+0.0025$; within-replicate Monte Carlo se
$0.0007$, recomputed for the randomized estimator) and with the
earlier interpolated plug-in ($+0.0024$). The supported statement is
conditional: the final design's average advantage over calibrated no
borrowing is \emph{positive in this conditional analysis} --- about a
quarter of a percentage point of power, positive in every examined
split of this validation batch and in $499$ of $500$ mixing-weight
bootstrap resamples re-run under randomized matching at the fixed
common target ($95\%$ interval $[+0.0005, +0.0045]$) --- with the
uncertainty sources examined \emph{separately} (split and threshold
estimation; finite simulation; mixing-weight estimation), not jointly
propagated, no independent replicate batch generated, and the
deconvolution's normal form unexamined beyond the round-5 sensitivity
family; a population-level or cross-batch claim is not made. Under a conservative
...
of that legacy interpolated analysis $[+0.0006, +0.0044]$, against
$[+0.0005, +0.0045]$ for the randomized estimator above) and rise
steeply as the assumed
drift distribution narrows ($+0.013$ under $N(0,1)$, $+0.040$ under
$N(0,0.5)$, $+0.077$ at no drift). The randomized-boundary repeated
cross-fit reproduces the dispersion gradient at the common
target on the identical splits --- $+0.0128$ under $N(0,1)$ and
$+0.0397$ under $N(0,0.5)$, every split positive --- and, because
those mixings are centred at zero rather than at the corpus mean, they
vary location as well as dispersion; holding the location at the
corpus mean $+0.124$ and narrowing only the SD gives $+0.0117$ and
$+0.0362$ on the same splits, so the gradient is dispersion-driven.
The dependence on drift dispersion (the
average value of borrowing falls as dispersion grows, and this corpus's
dispersion is wide) is thus stable across the examined splits, not
only a plug-in
pattern. What
the analysis supports,
exploratorily, is that at this corpus's drift dispersion the final
design's size-standardized average gain is positive in this
conditional analysis but an order of magnitude
smaller than under narrow drift, and that what it exhibits beyond that
small average is an observed local ROC-efficiency advantage. The frozen-threshold profile differences
...
the diagnostic locates an observed local advantage at small drift of
either sign, with \emph{asymmetric} magnitude: the final design's
alternative-world rejection advantage over no borrowing is $+0.07$ near
zero drift ($+0.069$ at $0$, $+0.074$ at $-0.25$, $+0.054$ at $+0.25$),
fades faster on the positive side ($+0.057$ at $-0.5$ versus $+0.020$ at
$+0.5$; $+0.022$ at $-0.75$ versus $+0.007$ at $+0.75$), and reverses
beyond $|{\rm drift}|
...
\caption{In-validation size-standardized reanalysis (exploratory;
thresholds
estimated on the validation batch itself, unlike
Table~\ref{tab:calibrated}, which evaluates the original frozen
deterministic decision rule --- a different estimand). Upper table:
the main analysis --- randomized-boundary repeated cross-fit, one row
per mixing measure and target (training-half size exact,
evaluation-half average size approximately matched; central $95\%$
split-quantile ranges are descriptive and conditional on this batch;
the two location-held rows keep the corpus mean $+0.124$ and narrow
only the SD). Lower table, first block: legacy full-batch
deterministic-threshold interpolation to
no-borrowing's validated size $0.0251$ (approximately size-adjusted;
retained for continuity --- its deterministic repeated-split version
centres at $-0.0007$, a granularity artifact dissected in
Supplement~S2). Middle:
conservative
calibration at the smallest threshold whose MC $95\%$ upper bound of
average type~I is $\le 0.025$. Bottom: mixing-measure plug-in point
estimates and the cell-specific ROC-efficiency
...
\hline
\multicolumn{6}{l}{\emph{Randomized-boundary repeated cross-fit
(final design vs calibrated no borrowing; $R = 200$ half-splits)}}\\
Mixing & Target & Mean $\Delta$ & Central $95\%$ range &
Eval.\ size (no borrow/final) & Splits $>0$ \\
\hline
corpus (deconvolved) & 0.02513 & $+0.0025$ & $[+0.0018,+0.0032]$ &
$0.02514$/$0.02514$ & 200/200 \\
corpus (deconvolved) & 0.025 & $+0.0025$ & $[+0.0017,+0.0032]$ &
$0.02502$/$0.02501$ & 200/200 \\
$N(0, 1)$ & 0.02513 & $+0.0128$ & $[+0.0122,+0.0136]$ &
$0.02514$/$0.02513$ & 200/200 \\
$N(0, 0.5)$ & 0.02513 & $+0.0397$ & $[+0.0383,+0.0407]$ &
$0.02515$/$0.02515$ & 200/200 \\
$N(+0.124, 1)$, loc.\ held & 0.02513 & $+0.0117$ &
$[+0.0111,+0.0125]$ & $0.02514$/$0.02513$ & 200/200 \\
$N(+0.124, 0.5)$, loc.\ held & 0.02513 & $+0.0362$ &
$[+0.0351,+0.0372]$ & $0.02515$/$0.02514$ & 200/200 \\
\hline
\end{tabular}
...
(se) \\
\hline
\multicolumn{5}{l}{\emph{Legacy full-batch deterministic-threshold
interpolation (approximately size-adjusted):}}\\
...
\multicolumn{5}{l}{\quad bootstrap of mixing weights, legacy
interpolated estimator: $95\%$ interval $[+0.0006, +0.0044]$}\\
\multicolumn{5}{l}{\quad (randomized estimator, common target:
$[+0.0005, +0.0045]$; see the first block and text)}\\
...
the redundancy measurement. This table evaluates the original frozen
deterministic decision rule; the size-standardized comparison of
Table~\ref{tab:sizematched} is a different estimand, and its $+0.0025$
...
\begin{figure}[htbp]
  ...
  cell). Null-world rejection rate (top) and alternative-world rejection
  rate (middle) across the
  ...
  grid. The final design's alternative-world rejection-rate advantage ---
  a rejection-rate difference at unequal cell-specific sizes (its
  per-cell null-world rejection runs $0.024$--$0.036$), not a
  size-matched power gain --- concentrates at small-to-moderate
  non-negative drift; its null-world rejection rate stays below the
  tolerated $0.05$ ceiling everywhere.}
  \label{fig:profiles}
\end{figure}
...
$0.5$ then improved the paired predictive score directly where the
uncapped prior over-borrowed (within-query capped-versus-uncapped
intervals excluding zero on the primary rows and the forward tier; on
the title-screened tiers the direction is inconclusive), left the capped
forward and title-screened deltas significant against the EB reference
(alongside the exploratory five-seed ensemble), and moved the primary analysis from
harmful to not detectably different from the EB reference --- gaining where
...
the predecessor ($+0.148$ against EB) is reduced to no detectable
difference ($+0.003$); superiority where information is rich remains directional rather
...
real data it is not detectably different from the EB reference in the
...
and is therefore not an implementable power benefit --- locates an
observed local ROC-efficiency advantage at small drift of either sign,
with asymmetric magnitude ($\approx +0.07$ near zero, $+0.057$ at $-0.5$
versus $+0.020$ at $+0.5$, reversing beyond
$|{\rm drift}| \approx 1$); \emph{averaged over the drift actually
observed in this corpus, size-standardized}, the advantage is small
and positive in every examined split ($+0.0025$ under
randomized-boundary matching at a common target; conditional on the
validation batch and estimated mixing), an order of magnitude below
its value under
narrower drift ($+0.013$/$+0.040$), with the
dispersion gradient stable across the examined splits; \emph{under adversarial
...
audit-informed, title-screened binary-response subset --- does not
demonstrate a
...
not detectably different from the EB reference there --- and thus not
yet useful --- with the direct within-query capped-versus-uncapped
contrast showing a mean reduction ($-0.029$; block bootstrap $95\%$ CI
$[-0.053,-0.010]$). The favourable
real-data
evidence lives above the primary tier, in two populations that we keep
distinct: the full-corpus forward tier ($n=775$, no endpoint screening)
and the title-screened reruns (which inherit the subset's measured
$\approx 80\%$ strict-ORR purity); the capped design's deltas on these
tiers are significant against the EB reference; selective borrowing improves substantially
...
value is graded triage --- coarse risk grouping rather than a
per-query ranking --- recomputed under the cap for the final
design itself (correlation $+0.14$; group deltas
$+0.005/-0.051/-0.074$, the top group significant) --- above all the
elimination of forced-borrowing
...
in-validation part: size-standardized comparisons, whose thresholds
are estimated on the validation batch itself, give $+0.0025$ under
randomized-boundary matching at a common target (training-half size
exact; positive in all $200$ examined splits and in $499/500$
mixing-weight bootstrap resamples) --- small, exploratory, and
conditional on the batch and the estimated mixing ($+0.013$/$+0.040$
under narrower mixings; the deterministic-threshold diagnostic is in
Supplement~S2). The cell-specific
ROC-efficiency diagnostic locates an observed local advantage, asymmetric
in magnitude, at small
...
the evaluated retrospective datasets the direct within-query contrasts
show the cap \emph{reduced the mean harm} of learned borrowing on the
primary rows, \emph{improved an already favourable tier mean}
(tail-driven) on
the forward tier, and was \emph{directionally inconclusive} on the
title-screened tiers --- neither a
...
redistributed. The complete versioned reproduction package --- code for
the full pipeline together with lightweight summary tables and
manuscript figures --- accompanies the submission as supplementary
material; upon acceptance the corresponding tagged version will be
deposited in Zenodo and its DOI added here. The package includes the
per-round
analysis scripts (\texttt{scripts/},
\texttt{simulation/gold\_standard/}), their per-split and per-example
outputs (\texttt{results/tables/}), and the assertion-check scripts
(\texttt{scripts/verify\_round*.py}) so that every reported number can
be re-verified. A release
...
between-design comparisons size-standardized by randomized-boundary
tests at a common target (training-half size exact; evaluation-half
average size approximately matched). & Exploratory throughout; not a
