...
def main():
    ...
    ax2.step(xr, np.arange(1, len(xr) + 1) / len(xr), where="post",
             color="#1b9e77", lw=1.9,
             label="randomized boundary, common target")
    ...
    ax2.set_title("b  Randomized-boundary size standardization, common target\n"
                  "(training-half size exact; medians and central 95% ranges)",
                  fontsize=10)
