...
five-fold-larger batch; between-design comparisons size-matched exactly
in expectation by randomized-boundary tests at a common target).
...
size-adjusted average power advantage over calibrated no borrowing that
is small but consistent once size is matched exactly in expectation
(randomized-boundary repeated cross-fitting at a common target:
$+0.0025$, positive in all $200$ random splits; deterministic
thresholds had hidden it behind size granularity at $-0.0007$), rising
to $+0.013$/$+0.040$ under narrower $N(0,1)$/$N(0,0.5)$ mixings. A cell-specific ROC-efficiency
...
exploratory and corpus-specific: exactly size-matched, the advantage at
this corpus's drift dispersion is about a quarter of a percentage point
of power, an order of magnitude below its value under narrower drift. All claims are graded by frame in the text; expert review,
...
exactly-size-matched mixture-average gain in simulation ($\approx 0.25$
...
masquerade as a power benefit; equality is enforced \emph{exactly in
expectation} by randomized-boundary tests at a common target (the test
...
atom ($n=281$) yields a mean benefit of $+0.074$ (CI $[+0.027,+0.120]$)
against $+0.080$ (CI $[+0.040,+0.120]$) for the pre-specified $I \ge 8$
rule ($n=291$) under the same capped scoring --- the learned triage
still matches, and does not beat, the hand rule. Where this paper
...
below establish --- centres the final
design's size-adjusted difference near zero: mean $-0.0007$, median
$-0.0009$ (split SD $0.0011$; central $95\%$ split-quantile range
$[-0.0014, +0.0039]$; only $4\%$ of splits positive; the mean sits at
about the $75$th split percentile, above the median, because $8$ of the
$200$ splits form a separated positive tail reaching $+0.0057$ --- shown
and dissected in Supplement~S2, Figure~S1). Because the $200$
...
does not include between-batch sampling variability. The split
distribution's shape is itself diagnosable (Supplement~S2 and
Figure~S1): the split estimate correlates $0.91$ with the
evaluation-half size gap between the two calibrated tests; the
no-borrowing threshold takes only four distinct values over the $400$
fold-halves (a discrete test statistic), so it lands slightly oversized
on the evaluation half in typical splits (average $0.0257$ against the
$0.0251$ target) and undersized in exactly the eight tail splits
...
tail and non-tail splits. Both the negative core and the positive tail
are therefore size-matching granularity of the discrete no-borrowing
threshold, not a cell-specific power structure --- and no-borrowing's
typical $+0.0006$ evaluation-half liberality is of the same order as
the effect being estimated, so the deterministic-threshold analysis
does not deliver an equal-size comparison at this resolution. Exact
size matching requires the classical randomized construction, and we
re-ran the analysis that way: on each training half, for each design,
the test rejects above a critical value $c$ and with probability
$\gamma$ \emph{at} $c$, with $(c, \gamma)$ solving the weighted
training-half size exactly; $(c, \gamma)$ is then fixed and applied to
...
at a \emph{common} target for both designs under every mixing. The
result reverses the deterministic-threshold picture: under the corpus
mixing at target $0.02513$ the size-matched difference is $+0.0025$
(median $+0.0025$; split SD $0.0004$; central $95\%$ split-quantile
range $[+0.0018, +0.0032]$; positive in \emph{all} $200$ splits; both
designs' evaluation-half sizes average $0.02514$), and the same at
target $0.025$ ($+0.0025$) --- indistinguishable from the full-batch
plug-in $+0.0024$, whose interpolation had approximated the same
randomization all along. The deterministic-threshold reanalysis was
approximately size-adjusted at best, and its near-zero centre reflected
the no-borrowing test's residual liberality, not the absence of a
difference. We therefore replace the previous conclusion: under exact
expected-size matching, the final design's average advantage over
calibrated no borrowing on this corpus is \emph{small but consistent}
--- about a quarter of a percentage point of power ($+0.0025$;
within-replicate Monte Carlo se $0.0007$; weight-bootstrap interval for
the estimated mixing itself $[+0.0006, +0.0044]$) --- exploratory,
conditional on this validation batch, and with mixing-measure
estimation error and the deconvolution's
normal form unpropagated on top. Under a conservative
...
cross-fit reproduces the dispersion dependence cleanly at the common
target on the identical splits --- $+0.0128$ under $N(0,1)$ and
$+0.0397$ under $N(0,0.5)$, every split positive (the earlier
deterministic-threshold reruns had used mixing-specific targets, which
mixes size differences into a dispersion comparison; the common-target
reruns remove that) --- so the dependence on drift dispersion (the
average value of borrowing falls as dispersion grows, and this corpus's
dispersion is wide) is robust under repetition, not only a plug-in
pattern. What
the analysis supports,
exploratorily, is that at this corpus's drift dispersion the final
design's size-matched average gain is real but an order of magnitude
...
re-estimated per split, evaluation out of fold) with deterministic
thresholds centres the final
design's difference at $-0.0007$ (central $95\%$ split-quantile range
$[-0.0014, +0.0039]$) --- an artifact of threshold granularity: with
exact expected-size matching by randomized-boundary tests at a common
target the difference is $+0.0025$ (split SD $0.0004$; range
$[+0.0018, +0.0032]$; positive in all $200$ splits), matching the
full-batch interpolation $+0.0024$ (see text). Middle:
conservative
...
(dispersion dependence robust under repetition; see text) and the cell-specific ROC-efficiency
...
forward and title-screened deltas as the tiers' only significant
selective-versus-EB intervals, and moved the primary analysis from
...
observed in this corpus, exactly size-matched}, the advantage is small
but consistent ($+0.0025$, positive in all $200$ random splits under
randomized-boundary matching at a common target; the
deterministic-threshold reanalysis had centred near zero because of
threshold granularity), an order of magnitude below its value under
narrower drift ($+0.013$/$+0.040$), with the
dispersion dependence robust under repetition; \emph{under adversarial
...
tiers are the evaluation's only significant
...
per-query value is triage, recomputed under the cap for the final
design itself (correlation $+0.14$; graded group deltas
...
estimated on the validation batch itself, gave positive plug-in
estimates
throughout ($+0.002$ to $+0.006$); deterministic-threshold repeated
splitting initially centred the difference near zero, but that centring
was threshold granularity (the discrete no-borrowing test sat slightly
liberal on the evaluation halves), and under exact expected-size
matching by randomized-boundary tests at a common target the difference
is $+0.0025$ --- positive in all $200$ random splits, matching the
plug-in, small in absolute terms, exploratory, and conditional on the
batch and the estimated mixing ($+0.013$/$+0.040$ under narrower
$N(0,1)$/$N(0,0.5)$ mixings). The cell-specific
...
between-design comparisons size-matched exactly in expectation
(randomized boundary, common target). & Exploratory throughout; not a
