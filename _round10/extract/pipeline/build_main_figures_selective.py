...
LAB = {
    "selective": "Selective (uncapped ablation)", "eb": "EB (intercept-only)",
...
def f2():
    ...
    # final design (capped selective) on the same tier: per-window capped
    # deltas vs EB and the pooled capped NLL, from the round-9 rescoring
    r9 = json.load(open(ROOT / "artifacts_round4" / "stress" /
                        "round9_tail_robustness.json"))["fwd_windows_vs_eb"]
    CAPPED_FWD_NLL = sum(r9[w]["capped_nll"] * r9[w]["n"] for w in r9) / \
        sum(r9[w]["n"] for w in r9)
    check("F2 final design pooled NLL", CAPPED_FWD_NLL, 2.8027, 2e-3)
    check("F2 final design w3 delta", r9["w3"]["capped_delta"], -0.0452, 2e-3)
    LAB["final_design"] = "final design (cap 0.5)"
    C["final_design"] = "#1b9e77"
    ...
    barh_panel(axL, FWD, DESIGN, mean(FWD, "eb"),
               ...
               extra=[("final_design", CAPPED_FWD_NLL)])
    ...
    for i, (label, n, d, lo, hi) in enumerate(per):
        axR.errorbar([i - 0.13], [d], yerr=[[d - lo], [hi - d]], fmt="s",
                     color=HL, ms=5, capsize=3, lw=1.3,
                     label=LAB["selective"] if i == 0 else None)
        cw = r9[("w1", "w2", "w3")[i]]
        cd, (clo, chi) = cw["capped_delta"], cw["capped_ci"]
        axR.errorbar([i + 0.13], [cd], yerr=[[cd - clo], [chi - cd]], fmt="o",
                     color=C["final_design"], ms=6, capsize=3, lw=1.5,
                     label=LAB["final_design"] if i == 0 else None)
        axR.text(i, max(hi, chi) + 0.004, f"n={n}", ha="center", fontsize=7)
    ...
    axR.set_ylabel("Paired $\\Delta$NLL vs EB (95% CI)")
...
def f3():
    ...
    fig_lab = {"eb": "EB reference", "selective": "Selective (uncapped ablation)",
    ...
    fig.suptitle("The uncapped selective prior approaches EB-level calibration at lower NLL\n"
                 "(final-design calibration in the diagnostics table)",
                 fontsize=11, fontweight="bold", y=1.05)
    fig.tight_layout()
...
def f12():
    ...
    ax.hlines(0.5, ax.get_xlim()[0], ax.get_xlim()[1], color="#1b9e77", ls="--", lw=1.2)
    ax.text(0.06, 0.515, "final-design cap 0.5 (applied at scoring)", fontsize=7.5, color="#1b9e77")
    ...
    ax.set_title("Uncapped learned total borrowing tracks candidate-set information\n"
                 "(mechanism diagnostic; Spearman +0.59, forward windows)",
