...
directly measured to reduce (within-query capped-versus-uncapped
...
median $-0.0009$; $95\%$ split interval $[-0.0014,+0.0039]$). A cell-specific ROC-efficiency
...
--- supported by direct paired capped-versus-uncapped intervals on the
primary rows and the forward tier, and by favourable point estimates with
intervals including zero on the title-screened tiers;
...
zero, so on these rows the cap reduced the harm --- consistent with, but
not independent
...
before any of these numbers were computed) leaves the upper-bound
results at least as favourable: pooled forward $\Delta$ versus EB
...
$281$ of $775$ queries), while on the two title-screened tiers the capped
point estimates are more favourable but the paired intervals include zero
(retrained $-0.0147$, block $[-0.0371,+0.0054]$; frozen $-0.0108$, block
$[-0.0260,+0.0029]$), so there the supported statement is only that the
cap did not detectably tax the benefit. Restricting historical mass acts
as regularisation against over-confident allocations out of time;
measured directly, the harm-attenuating mechanism improved the forward
tier and cost nothing detectable on the title-screened tiers. One
...
$-0.0009$ (split SD $0.0011$; $95\%$ split interval
...
$200$ splits form a separated positive tail reaching $+0.0057$ --- full
quantiles and histogram counts in Supplement~S2). Because the $200$
splits are half-splits of the \emph{same} validation batch, the split
interval characterises split-to-split and threshold-estimation
...
does not include between-batch sampling variability. The
...
intervals excluding zero on the primary rows and the forward tier;
title-screened point estimates favourable with intervals including
zero), left the capped
...
contrast favouring the cap ($-0.029$; block bootstrap $95\%$ CI
...
cap \emph{attenuated} the observed harm of learned borrowing (directly,
by within-query paired intervals, on the primary rows and the forward
tier; by favourable point estimates with intervals including zero on the
