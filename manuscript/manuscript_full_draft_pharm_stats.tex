...
frame in which its error rates are controlled. Textual similarity between
registry records is not sufficient for the first, and --- since strict
worst-case type~I control provably forfeits the power gains of borrowing ---
...
converts ClinicalTrials.gov records into a beta-binomial mixture prior. Its
core is a \emph{selective} two-head DeepSets model that learns, per
retrieved candidate, a mixture weight and a separate information discount,
and, at the set level, the per-query total borrowing --- anchored at an
intercept-only empirical-Bayes (EB) prior that also serves as the reference
for every real-data comparison. The complete specified \emph{final design}
adds a borrowed-mass cap fixed a priori at $0.5$ and contains no
outcome-adaptive component (a SAM-style adapter is retained only as a
sensitivity analysis). Evaluation separates design-time priors (scored by
held-out and forward predictive negative log-likelihood, NLL) from
outcome-adaptive layers (known-truth simulation only), and separates a
\emph{primary analysis} --- donor availability enforced at decision time by
a registry-availability proxy, on an audit-informed, title-screened ORR
subset --- from a hypothetical full-information upper bound. Decision
thresholds are calibrated in an empirical-mixture frame whose mixing
measure is the corpus-estimated donor--query drift distribution, subject
to a pre-specified worst-case ceiling of $0.05$, with thresholds selected
on one simulation batch and re-evaluated on an independent five-fold-larger
batch.

\noindent\textbf{Results.} In the primary analysis the final design is safe
but not yet useful: pooled $\Delta$NLL versus the availability-consistent EB
reference $+0.006$ ($95\%$ CI $[-0.005,+0.016]$; engaged subgroup $+0.030$,
CI $[-0.021,+0.079]$), where the uncapped ablation is significantly harmful
($+0.035$; engaged subgroup $+0.173$, CI $[+0.040,+0.324]$) --- a failure
that seed-replication, equal-size downsampling, and a learning-curve
experiment attribute to the thin as-of-cutoff training pools, and that the
cap converts to neutrality. On the upper-bound tiers the capped design's
forward and title-screened deltas are the evaluation's only significant
selective-versus-EB intervals ($-0.041$, CI $[-0.063,-0.020]$; $-0.049$,
CI $[-0.074,-0.023]$; $-0.044$, CI $[-0.067,-0.022]$): capping the learned
borrowing improves out-of-time generalisation rather than taxing it. In the
known-truth simulation, worst-case calibration is degenerate for the
uncapped prior ($\tau^{*}=0.9999$, power $0.069$) and yields no material
gain for any design, as theory predicts; under the empirical-mixture frame,
on the independent validation batch at frozen thresholds, the final design
holds drift-averaged type~I error at $0.026$ (MC se $0.0005$; calibrated
no borrowing itself validates at $0.025$) with worst null cell $0.036$
(Clopper--Pearson $95\%$ bound $0.039$, ceiling $0.05$) and returns
mixture power $0.515$ against $0.509$ --- a paired net benefit of $+0.007$
(MC se $0.0007$) that concentrates where the premise holds ($+4$ to $+5$
points at drifts $0$ to $+1.0$; $+1.3$ points in mid-sized queries) and
reverses at strong negative drift. The
corpus-estimated drift distribution itself (SD $\approx 1.3$ logits) shows
why decision-time gains are hard: donors far from the query are common. An
external-control stress battery shows that scenario's safety belongs to the
EB anchor, not the method; a single-rater registry audit bounds numeric
extraction error (two errors, headline shifts $<0.001$ NLL) and motivates
the title-screened estimand.

\noindent\textbf{Conclusions.} On decision-time-available registry data,
predictive gains over a fitted marginal EB prior remain undemonstrated; the
final design's value there is bounded-cost insurance, its power case rests
on the mixture frame and matures with the registry, and no borrowing method
can do better under strict worst-case control. All claims are graded by
frame in the text; expert review, prospective validation, and
protocol-specific operating characteristics remain prerequisites for any
decision-informing use.
...
the evidence for each decision graded explicitly: the set-level decisions
(when to borrow, and how much in total, under an explicit cap) are the ones
our data support, while donor-level discrimination remains an architectural
capability that is not empirically demonstrated;
(iii)~\emph{the capped final design and its calibration frames}: a
borrowed-mass cap fixed a priori at $0.5$, a corpus-estimated donor--query
drift distribution --- to our knowledge the first such measurement on
registry ORR data --- used as the mixing measure of an empirical-mixture
threshold calibration with a pre-specified worst-case ceiling, an
independent large validation batch so that threshold selection and
error-rate assessment never share simulation data, and worst-case
reporting alongside, in explicit acknowledgement that strict unrestricted
type~I control provably forfeits all power gains from borrowing [28];
...
(Section~\ref{sec:cap}): the borrowed-mass cap there achieves the same
...
\subsection{Borrowed-mass cap and the final design}
\label{sec:cap}
...
call $m = \sum_i \lambda_i$ its \emph{historical mass}. The borrowed-mass
cap is the projection
\[
...
prior mass; the excess returns to the anchored weak component. The cap is a
...
\emph{total}. We fix $c = 0.5$ a priori: the historical evidence may at most
match, and never outweigh, the anchored weak component that represents the
current trial's own context --- an equal-footing rule in the spirit of
Pocock's caution about historical controls [1]. The value was fixed in the
previous revision, before any of the cap-sensitivity computations reported
here; the full cap--power--error curve over $c \in \{0.1,\dots,0.9\}$
...
(Section~\ref{sec:twohead}), EB anchor, borrowed-mass cap $c = 0.5$, and a
...
Primary analysis (proxy $\times$ title-screened) & title-screened windows 128/155/306; pools 221/336/444 & Registry-availability proxy intersected with the audit-informed title-screened ORR subset; final design (cap $0.5$) scored alongside the uncapped ablation; upper-bound analyses labelled hypothetical & strict ORR (title-screened) \\
...
operationalised as an \emph{audit-informed, title-screened ORR subset}
...
borrowing method can retain a power advantage over no borrowing; this is not
a defect of any particular architecture but a general result [28], which our
grid reproduces empirically. We therefore report the worst-case threshold
$\tau^{*}$ (the smallest threshold whose worst-case type I error over all
null cells is $\le 0.025$, computed from per-replicate posterior
probabilities, so any threshold is evaluable offline) for completeness, and
calibrate the final design in an \emph{empirical-mixture} frame instead. Its
...
drift-weighted average type I error is $\le 0.025$, subject to a
pre-specified hard ceiling: worst-case type I error at
$\tau_{\mathrm{mix}}$ over \emph{all} null cells (two-sided uniform and
partial-conflict) may not exceed $0.05$, twice nominal. Average control
under an empirically grounded mixing measure plus a bounded worst case is
what the mixture frame guarantees; it does not guarantee strict control at
any single adverse drift, and we say so wherever the design is recommended.
Thresholds and the cap were selected on a $2{,}000$-replicate-per-cell
selection batch and then re-evaluated, unchanged, on an independent
$10{,}000$-replicate-per-cell validation batch simulated under a fresh seed
base, reporting per-cell type I error, the worst cell with its
Clopper--Pearson $95\%$ upper confidence bound, and mixture power ---
so threshold selection and error-rate assessment never share simulation
data. Because the query sample size is ancillary (fixed by design in a real
trial), operating characteristics are additionally reported conditionally by
$n_0$ stratum.
...
enforces donor availability at decision time on the title-screened ORR
...
The estimand is restricted to the title-screened ORR subset
...
($+0.173$, CI $[+0.040, +0.324]$; win rate $38\%$). The cap therefore does on
decision-time data exactly what it does in simulation: it bounds the damage
an over-confident learned allocation can do, at these training-pool sizes
converting significant harm into neutrality, with cap sensitivity
$0.3/0.7$ giving $+0.002$ (CI $[-0.005,+0.008]$) and $+0.013$
...
safe but not yet useful there, the uncapped ablation is actively harmful,
...
are harmful to the same degree (engaged subgroup $+0.100$/$+0.162$/$+0.131$
over three draws), so the harm tracks pool \emph{size}, not
...
(Section~S2 for the curve), completing the attribution: small training
pools make the learned allocation over-confident, the cap bounds the
consequences, and pool growth --- registry maturation --- is what removes
the cause.

\begin{table}[htbp]
\centering
\caption{\textbf{Primary analysis} (registry-availability proxy
$\times$ title-screened ORR subset). Per window: queries, queries with at
...
control are shown for attribution. Positive $\Delta$ favours the EB
reference.}
\label{tab:primary}
\small
...
\begin{figure}[htbp]
  ...
  \caption{\textbf{Primary analysis forest plot.} Paired $\Delta$NLL versus
  the availability-consistent EB reference (negative favours the design)
  with bootstrap $95\%$ CIs, per window and pooled. The final design (capped
  selective, green) is neutral everywhere; the uncapped ablation (orange) is
  significantly harmful, concentrated in the engaged subgroup; the frozen
  canonical-model control (grey) is neutral, and the
  training-mechanism experiments of the text attribute the ablation's harm
  to thin as-of-cutoff training pools.}
  \label{fig:primary}
\end{figure}
...
analysis above. Restricting this upper-bound tier to the title-screened ORR
...
title-screened filtering $-0.033$, CI $[-0.068,+0.001]$), so endpoint
contamination was not driving it.
...
tiers with the borrowed-mass cap of the final design
...
and $-0.0380$, both significant). Restricting borrowed mass evidently acts
as regularisation against over-confident allocations out of time, so the
safety mechanism costs nothing on the favourable tiers --- the two
significant title-screened upper-bound intervals are the strongest real-data
evidence for the final design, and they remain upper-bound-tier evidence.
...
final design is the borrowed-mass cap, not SAM: the cap achieves a
...
\emph{First, the worst-case frame is a dead end for every borrowing method,
ours included.} Exact worst-case calibration (the empirical
$97.5\%$ quantile over all null cells) yields $\tau^{*} = 0.9999$ for the
...
reference's $0.448$ at its own $\tau^{*}$ --- and for every other design a
$\tau^{*}$ at which no material power advantage over no borrowing survives
(the largest apparent gain, $+0.7$ points at cap $0.1$, is within Monte
Carlo error). This is the expected outcome, not a surprise: power gains
from external information are typically impossible under strict type~I
control [28]. We therefore report $\tau^{*}$ for completeness and calibrate
the final design in the empirical-mixture frame of
Section~\ref{sec:goldsim}.

\emph{Second, the cap--power--error curve} (Table~\ref{tab:capcurve})
answers what the pre-committed cap value trades away. Along
...
references everywhere (favourable-band gain $+1$--$2$ points), while at the
pre-committed $c = 0.5$ the gain where donors are approximately
exchangeable reaches $+5$ points at the same mixture average --- the cap
buys conditional upside with bounded downside rather than mixture-average
performance. We retain $c = 0.5$ as fixed a priori
(Section~\ref{sec:cap}) rather than re-tuning on this curve; SAM added to
...
$\tau^{*}$: exact worst-case threshold (worst null cell $\le 0.025$), with
no-conflict power at $\tau^{*}$. $\tau_{\mathrm{mix}}$: mixture-calibrated
threshold (drift-weighted average type~I $\le 0.025$), with mixture power
and worst-case type~I at $\tau_{\mathrm{mix}}$; the pre-specified ceiling
...
Design & @$0.975$ & $\tau^{*}$ & Power$_{s=0}$ & $\tau_{\mathrm{mix}}$ &
Mix.\ power & Worst T1 \\
\hline
...
\emph{Third, the mixture frame delivers a small but genuine calibrated net
benefit, concentrated where the method's premise holds}
(Figure~\ref{fig:profiles}; Table~\ref{tab:calibrated} carries the
validated values, which are the ones we cite). At the frozen thresholds,
on the independent $10{,}000$-replicate batch, the final design's
drift-weighted average type~I error is $0.0258$ (Monte Carlo se $0.0005$;
calibrated no borrowing itself validates at $0.0251$ --- threshold
selection contributes about $\pm 0.001$ of average-error uncertainty, which
is the reason the validation batch exists), its worst null cell is
$0.0360$ (Clopper--Pearson $95\%$ upper bound $0.0392$; Bonferroni over
all $22$ null cells, $0.0416$ --- comfortably inside the pre-specified
$0.05$ ceiling), and its mixture power is $0.5154$: a paired net benefit
of $+0.0066$ (within-replicate MC se $0.0007$) over calibrated no
borrowing and $+0.0021$ over the EB anchor alone. The validated profile
shows where the average comes from: $+4.4$ to $+5.3$ points over no
borrowing at drifts $0$ to $+1.0$, near parity at small negative drift,
and a $4$--$7$-point deficit at strong negative drift, where borrowed-down
donors cost power --- insurance economics, not dominance. Conditionally by
query size (validation batch), the net benefit is $+0.0$ points in the
smallest stratum ($n_0 = 20$--$28$: too little internal data for the
learned allocation to help), $+1.3$ points in the middle stratum
($n_0 = 29$--$37$), and $+0.7$ points in the largest --- borrowing needs
enough internal data to be adjudicated, and its value peaks in mid-sized
trials rather than the smallest ones. The SAM variant validates
\emph{below} no borrowing ($-0.3$ points), completing the measured case
for its exclusion from the final design.
...
signal, and it is why the final design carries an explicit borrowed-mass cap
and mixture-calibrated threshold (Section~\ref{sec:goldsim-results}) --- and
why the outcome-dependent SAM layer remains reported as a sensitivity
analysis: something must bound what design-time features cannot see.
...
difference, and the demonstrated value is bounded-cost insurance}: making
...
harmful to neutral --- the profile of an insurance mechanism, gaining where
the premise holds and bounding losses where it fails. The learned mass ranks
...
case $0.070$), and it is the explicit borrowed-mass cap of the final design
that bounds it ($0.044$, within the pre-specified $0.05$ ceiling) while
average-case control comes from the mixture calibration --- while in the
...
borrowed-mass cap is a deterministic design-time projection, so the capped
selective prior is one and the same object in both evidence systems --- on
real data it is neutral in the primary analysis and significantly better
than the EB reference on the title-screened upper-bound tiers, and in
known-truth simulation it carries mixture-calibrated average type~I control
with a bounded worst case and a small validated mixture power benefit. What
...
with power benefit (impossible for any borrowing method [28]). A sixth
observation grades the effectiveness claim by frame, and we commit to this
grading wherever the method is described: \emph{conditional on approximate
donor--query exchangeability} (small non-negative drift) the final
design buys up to $5$ points of power over no borrowing at controlled
average error (validated: $+4.4$ to $+5.3$ at drifts $0$ to $+1$,
averaging about $+3$ over the $|{\rm drift}| \le 0.5$ band); \emph{averaged over the drift actually observed in this
corpus} the net benefit is small but positive; \emph{under adversarial
drift} no design-time method retains a benefit, ours included, and the cap
exists to bound the cost of being wrong.
...
prospective selection can stand alone: the borrowed-mass cap of the final
design is the component that bounds this failure mode (worst case $0.044$
against $0.243$ uncapped, within the pre-specified $0.05$ ceiling), the
...
borrowed-mass cap with a calibrated threshold; and a pinned, reproducible
...
audit-informed, title-screened ORR subset --- demonstrates \emph{no}
...
neutral there --- safe but not yet useful. The favourable real-data
evidence lives above the primary tier: the capped design's forward and
title-screened upper-bound deltas are the evaluation's only significant
...
selective prior is rescuable only degenerately ($\tau^{*} = 0.9999$, power
$0.069$) --- which is the general impossibility of [28], not a defect
specific to this architecture. Under the empirical-mixture frame, whose
mixing measure is the corpus-estimated drift distribution and whose
worst case is capped by construction at the pre-specified $0.05$ ceiling,
the final design carries a small validated net mixture-power benefit over
calibrated no borrowing, concentrated exactly where approximate
exchangeability holds and absent where it fails; thresholds were selected
and validated on independent simulation batches. The SAM adapter is
measured to be redundant under the cap and is retained only as a
sensitivity analysis; the frequentist statement we make for the final
design is average-case control under a documented, empirically grounded
mixing measure with a bounded worst case --- not strict control, which
nothing borrowing can offer with a power benefit.
...
Title-screened ORR subset & Queries and donors whose endpoint titles pass
...
borrowed-mass cap $0.5$, mixture-calibrated threshold; no outcome-adaptive
