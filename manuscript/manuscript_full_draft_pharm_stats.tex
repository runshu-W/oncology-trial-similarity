...
prior achieved held-out NLL $2.8716$ on a clean $20\%$ test split, versus
...
($\Delta$ $-0.0581$, CI $[-0.1170,-0.0026]$). Under leakage-free forward
validation on disjoint future windows, each scored by a model refit on
...
best-trained window ($\Delta$ $-0.0461$, CI $[-0.0861,-0.0083]$). The learned
...
Outcome-adaptive SAM layers attained still lower same-outcome NLL (selective +
SAM $2.79$) but are reported separately because such scores partly reflect
adaptation to the outcome being scored.
...
\begin{figure}[htbp]
  ...
  effective-sample-size discount $a$, with a set-level head that learns the
...
\end{equation}
The rule-based prototype fixes $\lamz = 0.2$ and a flat anchor
$\mathrm{Beta}(1,1)$: raw historical weights are normalized to the remaining
...
and is scored by a model whose training set excluded it --- the clean $20\%$
...
Borrowing head-to-head + calibration & clean test split 281; development set 1{,}407 & Test split leakage-free for all methods; development-set columns include training outcomes for learned rows and are labelled & ORR \\
Selective / two-head training & 80\%/20\% split (seed 20260603) & Fit on 80\%, validated on the untouched 20\% & ORR \\
Leakage-free forward validation & disjoint windows 181/204/390 (train 632/813/1{,}017) & Each window $(c_k,c_{k+1}]$ scored once by the model and EB anchor refit on data $\le c_k$ & ORR \\
...
every method on the clean $20\%$ test split ($n = 281$), on which no method
...
Table~\ref{tab:headtohead} reports every prior on the clean $20\%$ test split
...
--- consistent leads whose intervals narrowly include zero. Third, the
\emph{architecture matters}: against the fixed-budget predecessor the
selective model improves by $-0.0581$ (CI $[-0.1170,-0.0026]$), a significant
paired gain attributable to the learnable total borrowing and EB anchor,
and Section~\ref{sec:forward-results} shows the same gain holds
leakage-free out of time. These results do not show that individual
...
NLL (lower is better) on the clean $20\%$ test split ($n=281$; primary
...
\midrule
\multicolumn{4}{l}{\emph{Outcome-adaptive (labelled; not compared as equals)}}\\
two\_head\_sam & learned + SAM & 2.7746 & 2.7583 \\
selective\_sam & selective + SAM & 2.7859 & 2.7621 \\
rule\_sam & rule + SAM & 2.8748 & 2.8591 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
  ...
  \caption{\textbf{EB-referenced borrowing head-to-head (clean test split,
...
\subsection{Leakage-free forward validation on disjoint future windows}
...
leakage-free by construction. Outcome-adaptive SAM rows are excluded here by
...
the architectural gain replicates leakage-free). Per window, the selective
lead over EB was $-0.0062$ (CI $[-0.0874,+0.0855]$), $-0.0074$
($[-0.0704,+0.0565]$), and $-0.0461$ ($[-0.0861,-0.0083]$): significant only
in the most recent window, which is also the one whose refit saw the most
training data --- a learning-curve pattern consistent with triage quality
improving as the training pool grows. In the pre-specified
...
\emph{worse} than intercept EB on the test split, $+0.0209$). Marginal
references are, in short, hard to beat: the selective model's demonstrated
advantages over them are the significant most-recent-window win, the
significant architectural improvement, and the interpretable triage behaviour
of Section~\ref{sec:triage-results}, not an across-the-board dominance.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{F2_forward_validation}
  \caption{\textbf{Leakage-free forward validation on disjoint future
...
\begin{table}[htbp]
\centering
\caption{Forward validation on disjoint future windows (leakage-free,
...
We assessed calibration against the real held-out outcome on the clean test
...
\begin{table}[htbp]
\centering
\caption{Predictive calibration diagnostics on the clean test split
...
selective\_sam & 2.786 & 0.094 & 0.694 & 0.929 & 0.979 \\
two\_head\_sam & 2.775 & 0.172 & 0.598 & 0.907 & 0.996 \\
rule\_sam & 2.875 & 0.248 & 0.537 & 0.879 & 0.996 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
  ...
  \caption{\textbf{Predictive calibration (clean test split).} (a)~PIT
...
whenever any candidate passes its gate). It tracked the pre-specified
design-time information measure: mean borrowed mass was $0.310$ when $I < 8$
and $0.585$ when $I \ge 8$, with Spearman correlation $+0.59$ between borrowed
mass and $I$. The model therefore learned, from features alone, the triage
that the subgroup analysis of Section~\ref{sec:forward-results} imposed by
hand: borrow substantially when the retrieved set carries usable endpoint
information, and fall back toward the EB anchor when it does not
(Figure~\ref{fig:triage}).
...
\paragraph{Case 2 (withholding protects; clean test query).} A second,
...
a single query, where declining to borrow is itself the correct decision. (The
...
clean test-split query). When borrowing is not clearly warranted, the
...
Second, \emph{selectivity is what earns back the difference}: making the
...
of time, the only design-time prior at or below the EB reference, a
significant lead in the best-trained forward window, and calibration
approaching the EB reference at lower NLL --- with an interpretable triage
policy (borrowed mass tracking candidate-set information) as the mechanism.
Third, the benefit of candidate-level borrowing is \emph{concentrated, not
uniform}: directionally strongest in the pre-specified information-rich
subgroup, decisive in the outcome-conditioned mechanism diagnostic when donor
consensus was near the truth, and absent elsewhere --- which is exactly the
behaviour a selective architecture should convert into aggregate gains as
training data grow. Fourth, known-truth simulation indicated conflict
sensitivity --- the learned and SAM-adapted priors reduced borrowed influence
as prior-data conflict grew --- but these simulation analyses do not
...
ROC-AUC of every design-time method we evaluated fell below chance ($0.445$),
while UIP-JS,
...
layer as a second line of defence, evaluated by simulation, rather than as an
optional
sensitivity analysis. Third, the forward validation is leakage-free with
respect to outcomes and model fitting --- disjoint future windows, each scored
once by models and EB anchors refit on past-only data ($632$, $813$, and
$1{,}017$ training examples; seed $20260603$ throughout, reproducible up to
minor cross-platform numerical differences) --- but it does not yet enforce
donor-level \emph{availability at decision time}: a donor completed before a
cutoff may have posted its results only later, and the stored registry records
are current versions rather than as-of-cutoff snapshots, so endpoint text
edited after completion could differ from what a contemporaneous user would
have seen. Results-first-posted dates are extracted for every trial, and
re-running the forward analysis with donors restricted to
results-posted-before-cutoff is planned; the query side is protected (the
entire results section of each pseudo-query is removed before retrieval), but
full registry-snapshot reconstruction remains future work. Fourth, marginal
...
the aggregate selective-versus-EB intervals narrowly include zero, the
information-subgroup threshold, though fixed before the final window was
examined, derives from exploratory analysis, and all learned-model results
rest on a single training seed per fit; multi-seed stability reporting is
planned. Fifth, automated extraction from ClinicalTrials.gov may introduce errors
...
clean-test-split and disjoint-window forward NLL, calibration diagnostics,
robust-MAP and classical baselines, and gold-standard and external-control
simulations with known truth, alongside an extraction-quality audit. The
empirical accounting is deliberately unsparing. Fixed-budget borrowing ---
including our own earlier architecture and a tuned robust-MAP prior --- did
not beat the two-parameter marginal reference on average; the selective
architecture did match or modestly exceed it, improved significantly on its
fixed-budget predecessor in and out of time, won significantly in the
best-trained forward window, approached EB-level calibration at lower NLL,
and learned an interpretable design-time triage policy in which borrowed mass
tracks how much usable information the retrieved candidates actually carry.
The known-truth simulation shows the same mechanism from the other side:
borrowing helps under exchangeability, harms under conflict, and the adaptive
layers reduce borrowed influence as conflict grows, at the cost of elevated
type~I error relative to the rule baseline under strong conflict ($0.070$
versus $0.050$ at a nominal $0.025$).
