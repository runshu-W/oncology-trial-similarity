...
title-screened ORR subset of measured $\approx 80\%$ purity --- from a
...
ceiling ($0.060$ under anchor-precision stress, exceeding it), a
size-adjusted average power difference over calibrated no borrowing of
only $+0.002$ to $+0.006$ across mixing measures and estimators, and a
pointwise size-matched conditional advantage of $+7.4$ points at zero
drift that fades by drift $+0.75$ and reverses beyond --- real exactly
where approximate exchangeability holds, and only there. The wide corpus
...
exploratory, corpus-specific, and marginal at this corpus's drift
dispersion. All claims are graded by frame in the text; expert review,
...
decision-time data exactly what it does in simulation: it attenuates the
damage
...
$267$ blocks each). Restricting historical mass evidently acts
...
estimated on the validation batch itself. Interpolating on the full batch
(thresholds so every design's average type~I equals no-borrowing's
$0.0251$), two thirds of the difference disappears: $+0.0024$
...
liberality), $-0.0010$ for cap $0.5$ + SAM. To propagate the
threshold-estimation and interpolation error this conditions away, we
also cross-fitted: thresholds estimated on one half of each cell's
replicates and the difference evaluated on the other half (both folds
averaged) gives $+0.0057$ for the final design (per-fold MC se $0.0010$;
achieved sizes on the evaluation halves $0.0244$ and $0.0260$ against the
$0.0251$ target). The spread between the two estimators --- $+0.0024$
versus $+0.0057$ --- is itself the scale of the threshold-estimation
uncertainty that the quoted MC standard errors do not contain, so we do
not pin the size-adjusted average difference beyond ``small and positive,
of order $+0.002$ to $+0.006$''; mixing-measure estimation error (weight
bootstrap $[+0.0006,+0.0044]$) and the deconvolution's normal form remain
unpropagated on top. Under a conservative
...
constraint, since power falls in the threshold), the ordering is
unchanged (no borrowing
...
average under this corpus's drift profile, and that its real advantage is
\emph{conditional and local}. The frozen-threshold profile differences
...
error runs $0.024$--$0.036$ there), so we matched size \emph{pointwise}:
at thresholds interpolated per cell so both designs reject exactly
$2.5\%$ of that cell's null replicates (cross-fitted across batch halves;
achieved per-cell sizes $0.024$--$0.026$ for both), the final design's
alternative-world advantage over no borrowing is $+7.4$ points at zero
drift, $+5.1$ at $+0.25$, $+2.0$ at $+0.5$, $+1.4$ at $+0.75$ --- and
\emph{reverses} beyond ($-1.9$ at $+1.0$, $-3.2$ at $+1.25$, $-2.7$ at
$+1.5$; per-cell MC se $\approx 0.003$). The like-for-like conditional
advantage is therefore real but confined to drifts $\le 0.75$; the
apparent frozen-threshold gains at drift $\ge 1.0$ were local liberality,
and we no longer describe them as conditional power gains.
...
full-batch interpolation to no-borrowing's validated size $0.0251$; the
cross-fitted final-design estimate (thresholds from one batch half,
evaluation on the other) is $+0.0057$ (per-fold MC se $0.0010$; achieved
sizes $0.0244/0.0260$) --- the spread versus $+0.0024$ is the scale of
unpropagated threshold-estimation error. Middle: conservative
calibration at the smallest threshold whose MC $95\%$ upper bound of
average type~I is $\le 0.025$. Bottom: mixing-measure robustness and
pointwise (per-cell) size-matched differences.}
...
$0.4999$}\\
\hline
\multicolumn{5}{l}{\emph{Final design's size-adjusted $\Delta$ under
...
\hline
\multicolumn{5}{l}{\emph{Pointwise size-matched $\Delta$ vs no borrowing
(per-cell null rejection $= 0.025$, cross-fitted;}}\\
\multicolumn{5}{l}{\emph{per-cell MC se $\approx 0.003$)}: drift $0$:
$+0.074$; $+0.25$: $+0.051$; $+0.5$: $+0.020$; $+0.75$: $+0.014$;}\\
\multicolumn{5}{l}{\quad $+1.0$: $-0.019$; $+1.25$: $-0.032$; $+1.5$:
$-0.027$}\\
\hline
\end{tabular}
\end{table}
...
donor--query exchangeability} the final design has a real like-for-like
advantage --- at pointwise-matched per-cell size, $+7.4$ points at zero
drift declining to $+1.4$ at drift $+0.75$, reversing beyond ($-1.9$ to
$-3.2$ at drifts $\ge 1$); \emph{averaged over the drift actually observed
in this corpus, size-adjusted}, the exploratory net difference is marginal
($+0.002$ to $+0.006$ across corpus-derived mixing measures and
estimators) and grows
