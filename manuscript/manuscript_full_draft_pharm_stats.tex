...
% ---------------------------------------------------------------------
\begin{abstract}
\noindent\textbf{Background.} A central problem in oncology drug development is
estimating how well a treatment works---its objective response rate (ORR),
say---in a new trial that is small, single-arm, or restricted to a narrow
subgroup, where the data alone give an imprecise estimate. Bayesian historical
borrowing can sharpen the estimate by drawing on completed trials, but its
benefit depends entirely on \emph{which} historical evidence is used: borrowing
from non-comparable trials biases the estimate and distorts type~I error and
coverage. Textual similarity between trial records is insufficient for this
choice, because a borrowable trial must also be compatible in disease setting,
patient population, regimen, endpoint definition, follow-up, eligibility
criteria, result usability, information size, and prior-data conflict risk.

\noindent\textbf{Methods.} We developed an explainable \emph{three-stage}
pipeline---retrieve, rerank, borrow---that converts ClinicalTrials.gov records
into a Bayesian mixture prior. Stage~1 (retrieve) uses existing backends
(hashing, ClinicalBERT embeddings, and a deterministic section pool inspired by
SECRET) to assemble a high-recall candidate set; Stage~2 (rerank) scores
candidates on nine auditable, borrowing-relevant clinical and statistical
features and applies a conservative clinical gate; and Stage~3 (borrow) turns
each surviving candidate into a beta-binomial historical component. The central
methodological contribution is a two-head DeepSets model that treats the
retrieved candidates as an unordered set and emits, for each candidate,
\emph{both} a mixture weight \emph{and} a separate effective-sample-size
discount---keeping ``which donor to borrow from'' and ``how much information to
borrow'' as distinct, independently learned quantities. An optional SAM-style
adapter then down-weights historical components that conflict with the observed
data. Because expert borrowability labels do not exist at scale, evaluation was
retrospective and simulation-based: leakage-controlled pseudo-queries (completed
trials replayed with their outcomes hidden), held-out beta-binomial predictive
negative log-likelihood (NLL), true-date temporal validation, simulation
operating characteristics, baseline comparisons, feature ablation, and software
tests.

\noindent\textbf{Results.} In borrowing baseline comparisons on objective
response rate (ORR) pseudo-queries, the trained two-head mixture model achieved
mean negative log-likelihood (NLL) $3.0562$ versus $3.1800$ for the rule-based
mixture; the conflict-adapted rule baseline achieved $2.9755$, and the same SAM
adapter applied to the trained two-head prior achieved $2.9328$. Under
leakage-free forward validation---models trained only on trials completed before
a cutoff date and scored on strictly later trials---across three cutoffs, the
conflict-adapted priors showed lower future-fold NLL with paired bootstrap
95\% confidence intervals excluding zero, and improved calibration diagnostics
(better probability-integral-transform uniformity and near-nominal coverage).
...
are therefore fourfold, and we foreground the second as the central
...
(ii)~\emph{the two-head DeepSets model---the core contribution---which learns,
for each individual donor trial, a mixture weight and a separate
effective-sample-size discount as a permutation-invariant function of the whole
retrieved set, so that donor selection and per-donor information size become
decoupled, learnable quantities}; (iii) a transparent SAM-style conflict adapter
reported as a sensitivity mechanism rather than a qualified procedure; and
(iv) a label-free retrospective calibration protocol for evaluating
borrowability pipelines. We draw the
...
the evaluation protocol; Section~3 reports borrowing
...
(which donor) \emph{and} a separate effective-sample-size discount $a_i$ (how
...
we show is what makes the learned discount head responsive to comparability at
all. The contribution we defend is this integration together with a
...
candidate's mixture weight and effective-sample-size discount either by a
...
$i$, extract the endpoint observation $(y_i, n_i)$; the two-head DeepSets model
maps the whole set $\{x_j\}$ to a mixture weight $\lambda_i$ \emph{and} a
separate effective-sample-size discount $a_i$; build the beta component
$\mathrm{Beta}(1+a_i y_i,\ 1+a_i(n_i-y_i))$.
\item Assemble the mixture prior
$p(\theta) = \lamz\,\mathrm{Beta}(1,1) + \sum_i \lambda_i\,\mathrm{Beta}(\alpha_i,\beta_i)$.
\item \emph{(optional)} \textbf{SAM adapter} \emph{(existing [11])}: if the
observed outcome conflicts with $p(\theta)$, down-weight historical mass and
return it to the weak component.
\end{enumerate}
\end{algorithm}
...
SAM conflict adapter of Section~\ref{sec:sam}).
...
Section~\ref{sec:rerank}) are masked entirely, $\sum_i \lambda_i = 1-\lamz$, and
...
the mean held-out beta-binomial mixture predictive NLL (Section~\ref{sec:eval});
...
credible-interval coverage and a reliability slope.
...
\label{sec:goldsim}
The gold-standard simulation makes the borrowing decision itself---not merely
the resulting estimate---an object of evaluation with known truth, and is
reported following the ADEMP structure for simulation studies [25].
...
(Table~\ref{tab:design-oc}). Performance measures are bias, empirical standard error, RMSE and
...
predictive NLL under rolling-origin, train-on-past/test-on-future splits---each
model sees only trials completed on or before a cutoff date and is scored on
strictly later trials, so no future information can leak into fitting---defined
by true primary completion dates (cutoffs 2020-12-31, 2021-12-31, 2022-12-31;
...
the data, so that borrowing does no harm --- is what the gold-standard
...
The results support several bounded observations. First, in retrospective
...
average loss. Second, in three rolling-origin splits the per-fold-retrained
...
pooled robust prior. Third, known-truth simulation indicated conflict
sensitivity --- the learned and SAM-adapted priors reduced borrowed influence as
prior-data conflict grew --- but these simulation analyses do not
...
Fifth, the framework and its evidence are limited to binary endpoints;
applying the full retrieval and two-head pipeline to progression-free survival
at fixed time points and other endpoints remains to be demonstrated. Sixth, the SECRET-style
...
% ---------------------------------------------------------------------
\section{Conclusion}
We developed a reproducible methodology prototype that separates two questions
usually merged in historical borrowing: whether a past trial \emph{can be found}
(retrievability) and whether it \emph{should be borrowed from} (borrowability).
The core contribution is a two-head DeepSets model that, for each retrieved
candidate, learns a mixture weight and a separate effective-sample-size discount,
so that ``which donor'' and ``how much information'' are decided independently
rather than collapsed into one weight.

Because expert borrowability labels do not exist at scale, we evaluated the
pipeline without them, using held-out predictive NLL, calibration diagnostics,
leakage-free forward validation, a robust meta-analytic-predictive baseline, and
gold-standard and external-control simulations with known truth, alongside an
extraction-quality audit. Across the real-data analyses the two-head prior with
the SAM conflict adapter gave the lowest predictive NLL and the most favorable
calibration among the evaluated priors, and outperformed a tuned robust-MAP
prior; the known-truth simulation showed that it reduces borrowed influence when
the historical evidence conflicts with the data, at the cost of slightly higher
type I error than the rule baseline under strong conflict.

These are calibration and simulation results, not clinical ones. They do not
establish that any individual historical trial is appropriate to borrow from;
that decision still requires expert clinical and statistical review, external
validation, and protocol-specific operating-characteristics evaluation.
