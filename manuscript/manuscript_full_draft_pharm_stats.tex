...
methodological contribution is a \emph{selective} two-head DeepSets model:
candidate-level heads emit, for each retrieved candidate, a mixture weight and
a separate information discount, while a set-level head places the weak
component inside the same learnable softmax, so that ``which donor,'' ``how
much information per donor,'' and ``whether to borrow at all'' are all learned
per query from design-time features. The weak component is anchored at an
intercept-only empirical-Bayes (EB) beta prior fitted to training-era queries
only, and the same EB prior---a two-parameter marginal prior that uses no
candidate information---serves as the reference baseline for every real-data
comparison. Because expert borrowability labels do not exist at scale,
evaluation was retrospective and simulation-based, with a strict partition
between \emph{design-time} priors (fixed before the current outcome is seen),
scored by held-out beta-binomial predictive negative log-likelihood (NLL) on
leakage-controlled pseudo-queries and by disjoint-window forward validation
with past-only refits, and \emph{outcome-adaptive} layers (SAM-style conflict
adaptation), whose same-outcome predictive scores are partly mechanical and
which are therefore assessed by known-truth simulation operating
characteristics instead.

\noindent\textbf{Results.} On unit-corrected ORR pseudo-queries the selective
prior's evidence is reported in two tiers. The \emph{primary analysis} ---
donor availability enforced by a registry-availability proxy (results-first-posted
before each query's start date) on an audit-validated strict-ORR estimand ---
shows no demonstrated predictive gain on decision-time-available data:
availability is scarce (from $2\%$ of development-era queries to $31\%$ in
the latest window), the availability-consistently trained selective model is
significantly \emph{worse} than the availability-consistent EB reference
where borrowing engages (engaged subgroup $+0.173$, CI $[+0.037,+0.324]$,
$n=119$; pooled $+0.035$, CI $[+0.007,+0.067]$) because the as-of-cutoff
training pools are thin, and the well-trained canonical model under the same
filters is only neutral ($-0.002$, CI $[-0.015,+0.010]$). All remaining
real-data results are a \emph{hypothetical full-information upper bound}
(every posted result treated as available). In that upper bound the selective
prior achieved held-out NLL $2.8716$ on a $20\%$ held-out split, versus
$2.9012$ for the EB reference (paired $\Delta$ $-0.0295$, bootstrap $95\%$ CI
$[-0.0722,+0.0109]$) and $2.9297$ for the fixed-budget two-head predecessor
($\Delta$ $-0.0581$, CI $[-0.1170,-0.0026]$). Under forward validation on
temporally disjoint future windows, each scored by a model refit on
past-only data, the selective prior matched or modestly exceeded the EB
reference overall ($\Delta$ $-0.0266$, CI $[-0.0591,+0.0064]$), improved
significantly on the predecessor architecture ($\Delta$ $-0.0665$, CI
$[-0.0993,-0.0346]$), and outperformed EB significantly in the most recent,
best-trained window ($\Delta$ $-0.0461$, CI $[-0.0861,-0.0083]$; a controlled
same-window comparison confirms the training-size reading). A $2 \times 2$
factorial attributes the architectural gain to the interaction: neither the
EB anchor alone ($+0.0002$ vs EB) nor learnable borrowing on a flat anchor
($+0.0413$) helps --- only their combination ($-0.0266$). Results are stable
over five training seeds, and an exploratory five-seed ensemble prior
exceeds the EB reference on the pooled upper-bound forward windows ($2.8084$
vs $2.8440$; $\Delta$ $-0.0356$, CI $[-0.0670,-0.0059]$), with the window-3
lead surviving Bonferroni adjustment; restricting the upper-bound tier to
the strict-ORR estimand preserves the pattern. The learned
total borrowing behaved as design-time triage: mean borrowed mass was $0.31$
when the candidate set carried little usable endpoint information and $0.59$
when it was information-rich (Spearman $+0.59$ with a pre-specified
information measure), against a forced $\approx 0.79$ in the fixed-budget
predecessor; calibration moved to near the EB reference
(probability-integral-transform Kolmogorov--Smirnov $0.094$ versus $0.173$ for
the predecessor and $0.035$ for EB), with conservative interval coverage.
Outcome-adaptive SAM layers are reported in a separately labelled block and
assessed by simulation only, because same-outcome predictive scores partly
reflect mechanical adaptation to the outcome being scored.

In a gold-standard simulation with parameter-level exchangeability of latent
rates known by construction, evaluated in a single-arm design testing
$H_0\!:\theta \le 0.20$, the selective prior attained a type I error of
$0.024$ and the best power among type-I-controlled methods ($0.613$ versus
$0.536$ without borrowing) under no prior-data conflict, while full pooling
and the power prior reached type I errors of $0.584$ and $0.484$. Under
uniform whole-set conflict, however, the learnable total borrowing removes
the cap that the fixed budget had provided: the standalone selective prior's
type I error rose to $0.145$--$0.243$ at strong conflict, against $0.061$--$0.072$
for the fixed-budget predecessor, $0.050$ for the rule, and $0.047$ for
robust-MAP; the anchored SAM adapter mitigated --- but did not eliminate ---
the inflation ($0.070$ at the same shifts, $2.8\times$ nominal) while
retaining the no-conflict power advantage. Recalibrating complete designs
over a pre-specified two-dimensional conflict grid shows the standalone
selective prior cannot be rescued by any threshold; a borrowed-mass cap of
$0.5$ is the most effective safety mechanism (worst-case $0.044$), and at a
threshold matched to the no-borrowing reference's own worst case the capped
selective + SAM design retains power $0.521$ versus $0.536$ without
borrowing --- under strong grid-wide control, borrowing's net power benefit
vanishes, and we claim pointwise mitigation only, not verified frequentist
safety. Ablation showed that the prospective set-level conflict
signal did not improve donor discrimination (ROC-AUC changes $-0.000$ to
$+0.016$), but was necessary for the learned per-source borrowed-size
...
distance from the query rate --- a response that is flat again under the
selective architecture, whose adaptivity migrates entirely to the set level.
In an external control arm scenario with controls
drifted downward, full pooling inflated type I error to $0.210$ against $0.013$
for internal-control-only analysis; the fixed-budget learned prior held it to
$0.031$ while borrowing, and the selective prior learned to withhold external
controls almost entirely, matching the EB-only reference ($0.018$) --- an
outcome whose safety an anchor-misspecification battery shows to be the
anchor's, not the method's (a $-0.4$-logit mis-centred anchor inflates type
I error to $0.10$--$0.13$; control-rate heterogeneity alone lifts the
correct anchor to $0.04$--$0.05$).

\noindent\textbf{Conclusions.} The framework provides a reproducible
methodology prototype and an EB-referenced, leakage-partitioned,
availability-tiered evaluation protocol for oncology historical borrowing.
Its defensible contributions are a
selective borrowing architecture whose per-query total borrowing is learned
rather than fixed---significantly improving on fixed-budget candidate
borrowing and yielding an interpretable design-time triage policy---and two
findings of independent cautionary value: that predictive gains over a
properly fitted marginal EB prior have not been demonstrated on
decision-time-available data (they exist only in the hypothetical
full-information upper bound, and an availability-consistently trained model
is harmful where it borrows), and that against known-truth simulation the
complete method achieves pointwise mitigation of conflict inflation but not
strong type I control --- under grid-calibrated control, borrowing's net
power benefit vanishes. These analyses do not establish average superiority over marginal
priors, expert borrowability of individual historical trials, or clinical
decision validity, and do not replace expert clinical and statistical review,
prospective validation, or protocol-specific operating-characteristics
evaluation. An endpoint-unit defect discovered while auditing the extraction
was corrected and the full real-data evaluation was re-run on unit-corrected
data; the defect, its measured scope, and the effect of the correction are
documented in the text, and the simulation results were never affected by it.
...
\begin{figure}[htbp]
  ...
  total borrowing $1-\lambda_0$ per query --- followed by the SAM conflict
  adapter.}
  \label{fig:pipeline}
\end{figure}
...
(ii)~\emph{the selective two-head DeepSets model---the core
contribution---which learns, for each individual donor trial, a mixture weight
...
and whether to borrow at all become decoupled, learnable quantities}; (iii) an
EB-referenced, leakage-partitioned evaluation protocol that scores every
design-time prior against an intercept-only empirical-Bayes reference and
assesses outcome-adaptive layers (a transparent SAM-style conflict adapter) by
known-truth simulation; and
(iv) an honest empirical accounting, of independent cautionary value, of how
much candidate-level borrowing adds over marginal priors on registry-scale
oncology data --- on average little, and concentrated where the retrieved
candidate set is information-rich. We draw the
...
and~\ref{sec:calib-results} evaluate this model (\texttt{selective}) against
...
aggregate comparisons: the design-time candidate-information measure
$I = \sum_i \mathrm{gate}_i\, a^{\mathrm{rule}}_i n_i$ dichotomised at $I \ge 8$
...
Primary analysis (proxy $\times$ strict) & strict windows 128/155/306; pools 221/336/444 & Registry-availability proxy intersected with the strict-ORR estimand; upper-bound analyses labelled hypothetical & strict ORR \\
...
Extraction-quality audit & 7{,}173 trials (26{,}603 primary-endpoint arm rows); expert audit 80 stratified rows & Automated consistency checks + synthetic round-trip ($8/8$) + six-item expert registry check (Section~\ref{sec:limitations-data}) & All extracted counts \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Endpoint canonicalisation and the primary estimand.} The expert
audit (Section~\ref{sec:limitations-data}) showed that roughly one in five
admitted ``ORR'' rows is a neighbouring binary response endpoint. The primary
real-data estimand is therefore restricted to a \emph{strict-ORR} subset,
...
(conservative). Validated against the $74$ unambiguous expert-audited rows,
the canonicaliser attains $98.6\%$ accuracy ($100\%$ sensitivity for strict
ORR, $91.7\%$ specificity; its one false positive is an ORR-titled endpoint
whose registry definition is progression-free survival at six months ---
title--definition mismatches are invisible to a title-based rule and remain a
documented residual). Applied to the corpus it retains $1{,}028/1{,}407$
queries ($73.1\%$, consistent with the audit's design-weighted purity
estimate) and $84.0\%$ of donor observations; the full
binary-response-type corpus is retained as a sensitivity population. The
canonicaliser is released with the code and its audit-validation is
reproducible.
...
\subsection{EB-referenced borrowing comparisons}
...
\emph{selective} model is the only design-time prior below the EB reference:
test NLL $2.8716$, paired $\Delta$ $-0.0295$ (CI $[-0.0722,+0.0109]$) against
EB and $-0.0504$ (CI $[-0.1066,+0.0003]$) against the stratified EB variant
--- consistent leads whose intervals narrowly include zero (stable across
five training seeds: $-0.029$ to $-0.038$). Third, the
...
NLL (lower is better) on the $20\%$ held-out split ($n=281$; primary
comparison) and on the full development set ($n=1{,}407$; includes training
outcomes for learned rows and EB fits, reported for context). Design-time
...
\textbf{selective} & learned, EB anchor & \textbf{2.8716} & 2.8335 \\
...
To answer whether a standard robust prior would suffice, Table~\ref{tab:headtohead}
...
component and a vague component). Its best-tuned configuration reached test
NLL $3.043$ --- better than flat-anchored borrowing but above the plain EB
reference, and sensitive to its robustification weight, degrading to $3.198$
at $w = 0.9$ as a single pooled MAP over-borrowed
from homogeneously treated historical evidence. The comparison that matters
for practice is therefore not ``robust prior versus naive borrowing'' but
``does any candidate-level construction beat the marginal fit,'' and on that
comparison only the selective architecture qualifies.
...
\subsection{Forward validation on disjoint future windows (outcome-fit temporal split)}
...
the partition rule of Section~\ref{sec:eval}. Results are reported in two
tiers, in order of evidential weight: first the \emph{primary analysis},
which enforces the registry-availability proxy on the strict-ORR estimand;
then the \emph{hypothetical full-information upper bound}, in which every
posted result is treated as if it had been available at design time.

\paragraph{Primary analysis: registry-availability proxy, strict-ORR
estimand.} A donor observation is treated as usable for a query only if the
...
through $8.8\%$ and $20.1\%$ to $31.3\%$ across the three windows, and from
$0\%$ of pre-2015 starts to $36\%$ of 2023 starts. On the strict-ORR windows
($n = 128/155/306$), with training also restricted to what was posted by
each cutoff (pools $221/336/444$), the availability-consistently trained
selective model shows \emph{no benefit and measurable harm}: pooled
$\Delta$ versus the availability-consistent EB reference $+0.035$
(CI $[+0.007, +0.067]$), concentrated entirely in the $119$ queries where
borrowing engages ($+0.173$, CI $[+0.037, +0.324]$; win rate $38\%$) ---
with training pools this thin, learned borrowing misfires. Substituting the
anachronistically well-trained canonical models (fit on the full
$632/813/1{,}017$ pools) under the same scoring filters is neutral at best
(pooled $-0.002$, CI $[-0.015, +0.010]$; engaged subgroup $-0.011$, CI
$[-0.072, +0.049]$), and the same holds without the strict-endpoint
restriction ($-0.001$, CI $[-0.017, +0.016]$; available-donor subgroup
$-0.003$, CI $[-0.072, +0.072]$). The primary conclusion of the real-data
evaluation is therefore: \emph{no demonstrated predictive gain on
decision-time-available data}, with harm where an availability-consistently
trained model actually borrows; training-pool size is the binding
constraint, and registry maturation is what would relax it.

\paragraph{Hypothetical full-information upper bound.} The analyses below
treat all posted results as available and are reported as an upper bound on
what borrowing of registry results \emph{could} contribute as availability
matures; none of them carries decision-time evidential weight.
Pooled over the $775$ future trials (Table~\ref{tab:forward}), the selective
prior achieved NLL $2.8175$ versus $2.8440$ for the past-fit EB reference
(paired $\Delta$ $-0.0266$, $95\%$ CI $[-0.0591,+0.0064]$) and $2.8839$ for
...
analysis above. Restricting this upper-bound tier to the strict-ORR
estimand preserves the pattern (retrained strict-subset pooled $\Delta$
$-0.034$, CI $[-0.074,+0.007]$; frozen-model strict filtering $-0.033$,
CI $[-0.068,+0.001]$), so endpoint contamination was not driving it.
...
analysis at the head of this subsection for the availability-consistent
...
bootstrap $95\%$ CI. The ensemble row is exploratory. Learned rows and EB
...
\textbf{selective} & \textbf{2.8175} & $-0.0266$ $[-0.0591,+0.0064]$ \\
...
\paragraph{Availability-consistent detail (supporting the primary analysis).}
The non-strict full-corpus version of the availability analysis gives the
same picture as the primary analysis above and supplies its mechanics. Under
...
upper-bound analyses of this subsection measure what borrowing of
registry-posted results could contribute as availability matures; under the
proxy they are not yet realisable, and claims throughout the paper are
scoped accordingly.
...
donor-block bootstrap (blocks defined by each query's strongest gated donor;
$346$ blocks, largest $36$): the architectural gain and the window-3 delta
survive ($[-0.105,-0.030]$ and $[-0.089,-0.007]$), while the pooled
...
year 2023) --- consistent with the availability gradient above. The completed expert audit
...
distribution is discrete, an equal-tailed interval attains at least its
nominal level by construction, so all methods over-cover somewhat and we
characterize coverage precisely rather than calling it nominal. The
...
intervals for the observed count (discreteness makes these conservative by
construction). Design-time and outcome-adaptive blocks as in
...
\textbf{selective} & \textbf{2.872} & 0.094 & 0.683 & 0.925 & 0.975 \\
...
\begin{figure}[htbp]
  ...
  prior against the intercept-only EB reference, the fixed-budget two-head
  prior, and the weak-only prior. The selective prior approaches the EB
  reference's PIT uniformity (KS $0.094$ vs $0.035$) at lower NLL, while
  flat-anchored priors are systematically optimistic; coverage is
  conservative for all methods, partly because equal-tailed intervals on a
  discrete predictive distribution over-cover by construction.}
  \label{fig:calib}
\end{figure}
...
gains into design gains. With no prior-data conflict its type I error was
$0.024$ (nominal $0.025$; no-borrowing reference $0.037$) with power $0.613$
--- the best of any method that controlled the type I error, against $0.588$
...
non-selective controlled method. The outcome-adaptive layer is therefore not
an optional sensitivity analysis for the selective architecture but a
required companion: the selectivity that makes the prior efficient when
borrowing is warranted is precisely what makes it exploitable by conflict
that design-time features cannot see. What SAM provides is pointwise
mitigation, not strong control, and we quantify that distinction next.

\paragraph{Calibrated designs: the price of strong type I control.} Because a
required companion must be judged as part of a complete design, we
recalibrated the final decision threshold over a pre-specified two-dimensional
conflict grid --- uniform shifts $0$--$1.5$ at full conflict \emph{and}
partial-conflict cells in which only a fraction $f \in \{0.25, 0.5, 0.75\}$ of
donors is displaced (partial conflict creates within-set heterogeneity and is
easier: the standalone selective prior's worst partial-conflict cell reaches
$0.112$, versus $0.243$ under uniform displacement) --- storing per-replicate
posterior probabilities so that every design's rejection rate is available at
any threshold. We also evaluated a capped variant that projects the selective
prior onto a maximum borrowed mass of $0.5$. Three findings
(Table~\ref{tab:calibrated}). First, the standalone selective prior is not
rescuable by recalibration: no threshold in $[0.975, 0.9995]$ brings its
worst-case type I error over the grid to $0.025$. Second, distinguish two
targets. Against the literal nominal $0.025$ --- which even the no-borrowing
reference does not attain over the grid ($0.038$, by binomial discreteness) ---
calibration is punishing for every design: selective + SAM needs
$\tau^{*} = 0.9945$ and drops to $0.388$ power. Against the
\emph{reference-matched} target (worst-case no worse than no borrowing's own
$0.038$), the capped selective + SAM design calibrates gently
($\tau = 0.979$) and retains power $0.521$ versus the reference's $0.536$ ---
borrowing costs $1.5$ points of no-conflict power as the premium for matching
the reference's worst-case safety, and buys its benefits only where conflict
is absent. Third, the cap is the single most effective safety mechanism in
the comparison: uncalibrated worst-case type I error $0.044$ (capped, with or
without SAM) versus $0.243$ (uncapped standalone) and $0.070$ (uncapped +
SAM). We therefore do not describe the selective prior, with or without SAM,
as having verified frequentist safety; the verified statements are the
pointwise ones in Table~\ref{tab:design-oc} and the grid-calibrated ones in
Table~\ref{tab:calibrated}, and a deployment that requires strong control
over uniform-conflict scenarios should use the capped design at the
reference-matched threshold and expect no net power benefit from borrowing.

\begin{table}[htbp]
\centering
\small
\caption{Calibrated complete designs over the pre-specified two-dimensional
conflict grid (uniform shifts $0$--$1.5$ plus partial-conflict cells,
$f \in \{0.25,0.5,0.75\} \times$ shifts $\{0.5,1.0,1.5\}$; $2{,}000$
replicates per cell). Worst-case type I error is the maximum over all null
cells. $\tau^{*}$: smallest threshold with worst case $\le 0.025$ (none
exists for the standalone selective prior); $\tau_{\mathrm{ref}}$: smallest
threshold with worst case $\le$ the no-borrowing reference's $0.038$. Power
is at zero conflict. The nominal $0.025$ is below what the flat-prior test
itself attains over the grid, so the reference-matched column is the
operative comparison.}
\label{tab:calibrated}
\begin{tabular}{lcccccc}
\hline
 & Worst T1 & \multicolumn{2}{c}{Calibrated to $0.025$} & \multicolumn{2}{c}{Reference-matched} \\
Design & @$0.975$ & $\tau^{*}$ & Power & $\tau_{\mathrm{ref}}$ & Power \\
\hline
No borrowing              & 0.038 & 0.986  & 0.448 & 0.975  & 0.536 \\
EB-only reference         & 0.032 & 0.9825 & 0.448 & 0.975  & 0.498 \\
Rule + SAM                & 0.050 & 0.9895 & 0.426 & 0.9825 & 0.498 \\
Two-head + pro.\ + SAM    & 0.070 & 0.9925 & 0.412 & 0.9885 & 0.476 \\
Selective (standalone)    & 0.243 & ---    & ---   & ---    & ---   \\
Selective + SAM           & 0.070 & 0.9945 & 0.388 & 0.990  & 0.462 \\
Selective, cap $0.5$      & 0.044 & 0.988  & 0.428 & 0.979  & 0.519 \\
Selective, cap $0.5$ + SAM& 0.044 & 0.9875 & 0.435 & 0.979  & 0.521 \\
\hline
\end{tabular}
\end{table}
...
set-level conflict signal changed the borrowing-discrimination ROC-AUC (primary,
parameter-level labels) by $+0.017$, $+0.008$, $+0.006$, $+0.001$ and $+0.005$
across the five scenarios for the fixed-budget architecture, and by $+0.012$ to
$-0.000$ for the selective architecture ---
...
below chance ($0.43$--$0.45$ in scenario S4 under the primary labels; the
selective prior itself scored $0.429$), because surface features actively mislead
...
data, attained $0.813$ under the primary labels ($0.839$ under the
process-compatible labels) in the same scenario. We regard this as the honest
...
required companion for the selective prior (Section~\ref{sec:goldsim-results})
rather than an optional sensitivity analysis.

The signal does, however, do something else in the fixed-budget architecture,
which the borrowed-size response curve
(Figure~\ref{fig:ess-response}) makes visible. Without it, the learned discount
head is inert: the per-source borrowed sample size is flat across donors at
every true distance from the query rate ($43.6$, $43.7$, $43.8$, $43.9$ across
distance bins, pooled over the five scenarios). With the signal, the borrowed
size declines
monotonically with true distance ($31.3$, $30.4$, $28.0$, $23.6$). The
...
constant. The set-level signal supplies exactly that missing information
while never touching the query outcome.
...
selective model's discount response is flat ($43.6$, $43.7$, $43.8$, $43.9$):
once the total-borrowing head exists, training reallocates the comparability
response to the set level and the per-source discount stops grading donors.
Per-donor discrimination under the selective model is therefore no better
than under the fixed-budget model --- its gains come from deciding \emph{how
much} to borrow overall, not from grading \emph{whom} to borrow from more
finely --- and its protection against whole-set conflict rests entirely on
the total-borrowing head, which uniform displacement deceives, and on the
SAM layer.
...
\begin{figure}[htbp]
  ...
  Learned per-source borrowed sample size $a_k n_k$, binned by the donor's true
  distance $|\theta_k - \theta_0|$ from the query rate (pooled over the five
  estimation scenarios). A flat line means the discount head is inert. In the
  fixed-budget architecture the prospective signal is what activates the head
  (declining curve); in the selective architecture the response is flat even
  with the signal --- adaptivity migrates to the set-level total-borrowing
  head. The fixed-discount ablation is flat by construction.}
...
The defensible claim is therefore narrower than the one we set out to make,
and narrower still for the revised architecture: the prospective signal does
not improve donor ranking; in the fixed-budget model it is what makes the
per-source discount function at all; and in the selective model even that
role disappears, with the discount response flat and all adaptivity carried
by the set-level total-borrowing head.
...
and we no longer present it as evidence of method-level safety.
...
\paragraph{Case 2 (withholding limits harm; held-out test query).} A second,
independent example is
...
(true outcome 5 of 32, observed ORR $0.156$; a test-split query no model saw
in training). Stage~1--2 again admitted six endpoint-compatible historical
arms, but with modest usable information. Here borrowing is not clearly
warranted (Table~\ref{tab:case2}): the EB reference reaches NLL $3.026$, and
the fixed-budget model, forced to place $0.80$ of its mass on the candidates,
does \emph{worse} ($3.182$). The selective model borrows only $0.40$ of its
mass and lands within $0.021$ of the EB reference ($3.047$) --- the
triage behaviour of Section~\ref{sec:triage-results} operating at the level of
a single query: withholding does not beat the marginal reference here ---
the EB prior remains best --- but it avoids most of the penalty that the
forced budget pays. (The
rule prior happens to do well here, $2.978$, because its size-tilted weights
landed near the low observed rate; single queries carry no significance
either way.)

\begin{table}[htbp]
\centering
\caption{Second worked case study (NCT03046953, held-out ORR $5/32 = 0.156$;
held-out test-split query). When borrowing is not clearly warranted, the
fixed-budget prior is penalised for borrowing anyway; the selective prior
withholds most of its mass and stays close to the EB reference.}
\label{tab:case2}
\begin{tabular}{lrrc}
\toprule
Prior & Predicted ORR & Held-out NLL & 95\% interval covers truth \\
...
Read together, the two case studies show both halves of the intended
behavior: borrowing is taken up where compatible evidence sharpens the
held-out prediction (Case~1), and it is largely withheld where the marginal
reference is the better bet, limiting the damage that a forced borrowing
budget would otherwise cause (Case~2). Outcome-adaptive SAM rows are omitted
here under the partition rule of Section~\ref{sec:eval}; the aggregate
counterpart of this per-query behavior --- borrowing helping under
...
Second, \emph{selectivity is what earns back the difference, and its
demonstrated value is mostly harm avoidance}: making the
total borrowing learnable per query, anchored at the EB fit, produced a
...
approaching the EB reference at lower NLL. The learned mass ranks the
realized benefit of borrowing on unseen data (rank correlation $+0.16$,
...
exchangeability the selective prior converts predictive gains into the best
type-I-controlled power in our comparison, but its learnable total borrowing
...
its per-source discount response is flat, and control is restored only by
the anchored SAM companion, which itself mitigates rather than eliminates
the inflation --- while in the external-control world, trained
on a conflict-rich draw, the same head learns to withhold external controls
almost entirely, with operating characteristics that the
anchor-misspecification battery shows belong to the anchor. These
...
explicitly, because its two components are validated in different systems
that do not compose. The design-time selective component has a proper
out-of-sample real-data evaluation --- whose primary, availability-consistent
result is null-to-harmful --- but no standalone frequentist safety (its
uncalibrated conflict inflation reaches $0.243$ and no threshold rescues
it). The selective + SAM combination has known-truth simulation evidence ---
best pointwise no-conflict power among controlled designs, but mitigation
rather than strong control, and a vanishing net benefit once calibrated to
grid-wide control --- and \emph{cannot} have the same real-data predictive
evaluation, because SAM consults the outcome being scored. No single
evidence stream currently supports both practical gain and inferential
safety for the full method at once; we present the two partial validations
side by side and do not combine them into a claim about the complete
pipeline.
...
consistent with the canonicaliser's independent corpus-level rate of
$73.1\%$ (Section~\ref{sec:eval}). Agreement was
...
$78\%$ strict-ORR purity. Queries and donors carry the same heterogeneity,
...
prospective selection can stand alone, and treat the outcome-dependent SAM
layer, evaluated by simulation, as a required companion of the selective
prior rather than an optional sensitivity analysis --- while noting that the
companion mitigates rather than eliminates the inflation, that only a
borrowed-mass cap brings the worst case near the no-borrowing reference, and
that strong grid-calibrated control erases the net power benefit
(Table~\ref{tab:calibrated}). Third, the availability-controlled analysis of
Section~\ref{sec:forward-results} bounds what the forward validation can
...
the aggregate selective-versus-EB intervals include zero under every
resampling scheme we tried (query-level and donor-block bootstrap alike),
...
Section~\ref{sec:forward-results}. Fifth, automated extraction from ClinicalTrials.gov introduces errors
...
limitation, and endpoint-definition canonicalisation is the identified fix.
...
borrowed-size discount $a_i$ with reported ESS; simulation operating
...
total borrowing fraction --- so that ``which donor,'' ``how much information,''
and ``whether to borrow'' are decided independently rather than collapsed into
one fixed budget.
...
simulations with known truth, alongside automated and expert extraction
audits. The
...
audit-validated strict-ORR estimand --- demonstrates \emph{no} predictive
gain on decision-time-available data: the availability-consistently trained
model is significantly harmful where borrowing engages, and the well-trained
model applied under the same constraints is neutral. Everything favourable
about the real-data evaluation lives in the hypothetical full-information
upper bound: there, selective borrowing improves substantially on the
flawed fixed-budget predecessor --- in and out of time, under query-level and
donor-block resampling alike, under every training seed, and on the
strict-ORR subset --- single-fit selective priors are consistently but not
significantly ahead of appropriately fitted marginal EB references, an
exploratory five-seed ensemble does exceed the intercept-EB reference
($\Delta$ $-0.0356$, CI $[-0.0670,-0.0059]$),
the advantage is attributable to the interaction of the EB anchor with
learnable total borrowing, and
the demonstrated per-query value is triage, above all the
elimination of forced-borrowing harm where the candidate set carries little
usable information. That upper bound becomes realisable only as registry
reporting matures. The
known-truth simulation adds the design-side account: under exchangeability
the selective prior delivers the best type-I-controlled power in our
comparison ($0.613$ at a type I error of $0.024$), but its learnable total
borrowing removes the fixed budget's accidental cap under uniform whole-set
conflict (type I error up to $0.243$ standalone, unrescuable by threshold
recalibration), so
the outcome-adaptive SAM companion --- which mitigates the inflation (worst
case $0.070$, still $2.8\times$ nominal)
while keeping the no-conflict advantage --- is a required part of the
proposal, not a sensitivity analysis; and once complete designs are
calibrated to strong type I control over the pre-specified conflict grid,
the net power benefit of borrowing vanishes (capped selective + SAM $0.521$
versus $0.536$ without borrowing at matched worst-case safety). The
frequentist safety we can claim is pointwise mitigation under a documented
grid, delivered most effectively by a borrowed-mass cap; it is not verified
strong control.
...
Strict-ORR estimand & Queries and donors whose endpoint titles pass the
audit-validated canonicaliser. & Primary; full binary-response corpus is a
sensitivity population. \\
SAM adapter & Prior-data conflict adapter used as sensitivity analysis;
conflict ratio and returned mass defined against the prior's own weak
component. & Not a regulatory qualification. \\
\bottomrule
\end{longtable}
