...
adds a historical mixture-mass cap of $0.5$ (fixed before the cap
analyses) and contains no
...
subset --- from a hypothetical full-information upper bound. Operating
characteristics are assessed in a worst-case frame and in an exploratory,
corpus-specific decision analysis whose mixing
measure is the corpus-estimated donor--query drift distribution, with a
tolerated pointwise inflation ceiling of $0.05$, thresholds fixed
on one simulation batch, re-evaluation on an independent five-fold-larger
batch, and all between-design power comparisons made at matched validated
average size.

\noindent\textbf{Results.} In the primary analysis the final design is
predictively neutral but not yet useful: pooled $\Delta$NLL versus the
availability-consistent EB
...
experiment link to the thin as-of-cutoff training pools, and that the
...
uncapped prior ($\hat\tau^{*}=0.9999$, power $0.069$) and yields no
material gain for any design, as expected in this one-parameter setting.
An exploratory corpus-specific decision analysis --- drift-weighted
operating characteristics under the corpus-estimated drift distribution
(SD $\approx 1.3$ logits), thresholds frozen before an independent
five-fold-larger validation batch --- finds the final design's worst null
cell at $0.036$ (Clopper--Pearson $95\%$ bound $0.039$, tolerated ceiling
$0.05$) and, at matched validated average size, a mixture-power difference
over calibrated no borrowing of only $+0.002$ to $+0.006$ across
corpus-derived mixing measures (bootstrap interval $[+0.001, +0.004]$
under the canonical mixing): marginal on average, though rising steeply as
drift dispersion narrows ($+0.04$ under $N(0,0.5)$ drift) and
concentrated where approximate exchangeability holds ($+4$ to $+5$ points
at drifts $0$ to $+1$, where per-cell type~I error runs up to $0.036$).
The wide corpus drift distribution itself explains
why decision-time gains are hard: donors far from the query are common. An
external-control stress battery shows that scenario's protection belongs
to the EB anchor, not the method; a single-rater registry audit bounds
numeric extraction error (two errors, headline shifts $<0.001$ NLL) and
motivates the title-screened estimand.
...
final design's demonstrated value is bounding the predictive harm of
learned borrowing (with a tolerated, not eliminated, pointwise error
inflation of up to $0.036$ observed against the $0.05$ ceiling), while its
average power case is exploratory, corpus-specific, marginal at this
corpus's drift dispersion, and expected to mature with the registry. In
designs of this kind no borrowing method retains a power benefit under
strict worst-case control. All claims are graded by
...
Primary analysis (proxy $\times$ title-screened) & title-screened windows 128/155/306; pools 221/336/444 & Registry-availability proxy intersected with the audit-informed title-screened ORR subset; final design (cap $0.5$) scored alongside the uncapped ablation; upper-bound analyses labelled hypothetical & binary response-type, title-screened ($\approx 80\%$ strict-ORR purity) \\
...
and canonical alternatives) is therefore reported alongside, and all
comparisons between designs are additionally made \emph{size-matched}:
thresholds interpolated so that competing designs have identical validated
average type I error, so a difference in liberality can never masquerade
as a power benefit. Thresholds and the cap were fixed on a
$2{,}000$-replicate-per-cell selection batch and re-evaluated, unchanged,
on an independent $10{,}000$-replicate-per-cell validation batch under a
fresh seed base, reporting per-cell type I error, the worst cell with its
...
consequences, and pool growth --- registry maturation --- is the expected
remover of the cause.
...
and $-0.0380$, both significant). Restricting historical mass evidently acts
as regularisation against over-confident allocations out of time, so the
harm-bounding mechanism costs nothing on the favourable tiers --- the two
significant title-screened upper-bound intervals are the strongest real-data
evidence for the final design; they remain upper-bound-tier evidence, and
they inherit the title-screened subset's measured $\approx 80\%$
strict-ORR purity, so they are statements about the screened
binary-response estimand, not about a pure ORR population.
...
performance. We retain $c = 0.5$ as fixed before any of the round-4 cap
...
difference is $+0.0066$; \emph{size-matched} --- thresholds interpolated
so every design's validated average type~I error equals no-borrowing's
$0.0251$ --- two thirds of that difference disappears: $+0.0024$
...
liberality), $-0.0010$ for cap $0.5$ + SAM. Under a conservative
calibration requiring the Monte Carlo $95\%$ upper bound of average
type~I to sit below $0.025$, the ordering is unchanged (no borrowing
$0.5028$; final design $0.5055$; cap $0.3$ $0.5090$). The size-matched
gain is robust in sign but not in magnitude across mixing measures
(Table~\ref{tab:sizematched}): $+0.0010$ to $+0.0063$ under corpus-derived
alternatives (nonparametric raw and shrunken, era- and
availability-stratified, all-pairs; bootstrap of the mixing weights:
$95\%$ interval $[+0.0006, +0.0044]$), rising steeply as the assumed
drift distribution narrows ($+0.013$ under $N(0,1)$, $+0.040$ under
$N(0,0.5)$, $+0.077$ at no drift) --- the average value of borrowing is a
decreasing function of drift dispersion, and this corpus's dispersion is
wide. We therefore no longer describe the mixture analysis as
demonstrating a validated net power benefit: what it supports,
exploratorily, is that the final design gives up essentially nothing on
average under this corpus's drift profile, and that its material gains
are \emph{conditional}: $+4.4$ to $+5.3$ points over no borrowing at
drifts $0$ to $+1.0$ (where its own per-cell type~I error runs
$0.024$--$0.036$, the tolerated inflation made explicit), near parity at
small negative drift, and a $4$--$7$-point deficit at strong negative
drift. Conditionally by query size, the frozen-threshold net difference
...
\centering
\small
\caption{Size-matched exploratory decision analysis on the validation
batch. Top: mixture power at thresholds interpolated so each design's
validated average type~I error equals no-borrowing's $0.0251$ (paired
within-replicate MC se in parentheses), and at conservatively calibrated
thresholds (MC $95\%$ upper bound of average type~I $\le 0.025$).
Bottom: the final design's size-matched difference versus no borrowing
under alternative mixing measures --- sign-robust, magnitude-variable,
and steeply increasing as drift dispersion shrinks.}
\label{tab:sizematched}
\begin{tabular}{lccc}
\hline
Design & Size-matched power & $\Delta$ vs no borrow (se) & UCB-calibrated
power \\
\hline
No borrowing & 0.5088 & --- & 0.5028 \\
EB-only & 0.5073 & $-0.0015$ (0.0003) & 0.5028 \\
Selective, cap $0.1$ & 0.5114 & $+0.0026$ (0.0003) & 0.5055 \\
Selective, cap $0.3$ & 0.5144 & $+0.0057$ (0.0006) & 0.5090 \\
\textbf{Final design (cap $0.5$)} & 0.5112 & $+0.0024$ (0.0007) & 0.5055 \\
Cap $0.5$ + SAM & 0.5077 & $-0.0010$ (0.0006) & 0.4999 \\
\hline
\multicolumn{4}{l}{\emph{Final design's size-matched $\Delta$ under
alternative mixing measures}}\\
\multicolumn{4}{l}{corpus-derived: canonical $+0.0024$; nonparametric raw
$+0.0010$; shrunken $+0.0063$;}\\
\multicolumn{4}{l}{\quad era-stratified $+0.0025$; availability-stratified
$+0.0024$; all-pairs $+0.0011$;}\\
\multicolumn{4}{l}{\quad bootstrap of mixing weights: $95\%$ interval
$[+0.0006, +0.0044]$}\\
\multicolumn{4}{l}{canonical alternatives: $U(-1.5,1.5)$ $+0.0059$;
$N(0,1)$ $+0.0127$; $N(0,0.5)$ $+0.0403$;}\\
\multicolumn{4}{l}{\quad point mass at $0$ (no conflict) $+0.0767$}\\
\hline
\end{tabular}
\end{table}
...
$\times 4$ lifts the capped design to $0.059$ at shift $1.0$ --- at, and
marginally above, the tolerated ceiling --- echoing the external-control
...
signal, and it is why the final design carries an explicit mixture-mass cap
...
difference, and the demonstrated value is harm-bounding}: making
...
the premise holds and bounding (not eliminating: pointwise error inflation
up to the tolerated ceiling remains) losses where it fails. The learned
mass ranks
...
case $0.070$), and it is the explicit mixture-mass cap of the final design
that bounds it ($0.044$, within the tolerated $0.05$ ceiling; the
drift-averaged behaviour is characterised only in the exploratory
decision analysis) --- while in the
...
design buys up to $5$ points of power over no borrowing (validated:
$+4.4$ to $+5.3$ at drifts $0$ to $+1$ --- where its own per-cell type~I
error runs up to $0.036$, inside the tolerated $0.05$ ceiling but above
nominal); \emph{averaged over the drift actually observed in this corpus,
at matched validated size}, the exploratory net difference is marginal
($+0.002$ to $+0.006$ across corpus-derived mixing measures) and grows
only as drift dispersion shrinks; \emph{under adversarial
drift} no design-time method retains a benefit, ours included, and the cap
exists to bound --- at the tolerated ceiling, not at nominal --- the cost
of being wrong.
...
it as such: with thresholds frozen before an independent validation batch
and all comparisons made at matched validated size, the final design's
drift-averaged power difference over calibrated no borrowing is marginal
($+0.002$ to $+0.006$ across corpus-derived mixing measures), its
conditional gains concentrate where approximate exchangeability holds,
its pointwise error inflation is tolerated up to a declared $0.05$
ceiling (observed worst cell $0.036$) rather than controlled at nominal,
and the mixing measure itself is estimated from this corpus, not from a
deployment population. The SAM adapter is
measured to be redundant under the cap and is retained only as a
sensitivity analysis. What the evidence supports, in one sentence: the
cap bounds the predictive harm of learned borrowing, and a
decision-time benefit has not been demonstrated.
