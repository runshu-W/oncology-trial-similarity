...
mean negative log-likelihood (NLL) $2.8957$ versus $3.0100$ for the rule-based
mixture and $2.9006$ for the conflict-adapted rule baseline; the same SAM
adapter applied to the trained two-head prior achieved $2.7938$, the lowest of
all evaluated priors. Under
...
operating-characteristics evaluation. An endpoint-unit defect discovered while
auditing the extraction was corrected and the full real-data evaluation was
re-run on unit-corrected data; the defect, its measured scope, and the effect
of the correction are documented in the text, and the simulation results were
...
Leakage-free rolling-origin forward & train 632/813/1{,}017; future folds 775/594/390 & Train-on-past / test-on-future at cutoffs 2020/2021/2022-12-31; per-fold retraining & ORR \\
...
reported in Section~\ref{sec:calib-results}; Figure~\ref{fig:nll}). The weak-only prior had mean NLL $3.1641$
and the rule mixture baseline $3.0100$. The trained two-head mixture model
achieved mean NLL $2.8957$ (delta $-0.1143$ relative to rule), narrowly ahead
of the conflict-adapted rule baseline at $2.9006$ (delta $-0.1094$); the same
SAM adapter applied to the trained two-head prior achieved $2.7938$
(delta $-0.2162$), the lowest of all evaluated priors. MAP-like and
power-prior-like baselines had higher NLLs
($3.2219$ and $3.2269$, respectively). The full head-to-head ranking, including
...
\midrule
\textbf{two\_head\_sam} & learned + SAM & \textbf{2.7938} & 0.996 \\
two\_head & learned & 2.8957 & 0.974 \\
rule\_sam & rule + SAM & 2.9006 & 0.995 \\
robust\_map ($w{=}0.5$) & robust-MAP & 3.0019 & 0.986 \\
rule & rule & 3.0100 & 0.972 \\
commensurate\_like & classical & 3.0325 & 0.964 \\
robust\_map ($w{=}0.9$) & robust-MAP & 3.0996 & 0.937 \\
weak\_only & none & 3.1641 & 0.994 \\
map\_like & classical & 3.2219 & 0.945 \\
power\_prior\_like & classical & 3.2269 & 0.942 \\
\bottomrule
\end{tabular}
...
NLL $3.002$ --- marginally better than naive rule ($3.010$) and clearly better
than weak-only ($3.164$) borrowing, but worse than every conflict-adapted
mixture prior --- and it was sensitive to its robustification weight, degrading
to $3.040$ and $3.100$ at higher weights as a single pooled MAP over-borrowed
from homogeneously treated historical evidence.
...
\begin{figure}[htbp]
  ...
  pseudo-queries; the two-head + SAM mixture prior is lowest ($2.794$), ahead
  of a tuned RBesT-style robust meta-analytic-predictive (MAP) prior (best
  $3.002$). (b)~Empirical 95\% credible-interval coverage per method.}
  \label{fig:nll}
\end{figure}
...
evaluation restricted to the future fold, with 775, 594, and 390 held-out
...
mixture prior improved future-fold NLL over the rule baseline by $-0.1086$,
$-0.1049$, and $-0.1004$ nats at the three cutoffs, with paired bootstrap 95\%
confidence intervals $[-0.1313,-0.0880]$, $[-0.1267,-0.0833]$, and
$[-0.1275,-0.0752]$; all intervals exclude zero. Averaged across cutoffs,
held-out NLL was $2.852$ for rule-SAM, $2.956$ for rule, and $3.094$ for
weak-only, while map-like and power-prior-like baselines were worse ($3.156$ and
$3.160$), and 95\% interval coverage on the future fold remained near nominal
($0.94$--$1.00$). The trained two-head and two-head-SAM priors achieved the
lowest future-fold NLL ($2.868$ and $2.756$, with per-cutoff improvements over
rule of $-0.073$ to $-0.101$ without the SAM adapter and $-0.198$ to $-0.203$
with it); because each cutoff was scored with
a two-head DeepSets model retrained on past-only (date at or before the cutoff)
data, these learned-model rows are leakage-free as well, with all
...
improving mean NLL over the rule baseline by $-0.215$ to $-0.219$ nats.
...
\begin{figure}[htbp]
  ...
  cutoffs; two-head + SAM is lowest ($2.756$). (b)~Per-cutoff improvement over
...
\midrule
\textbf{two\_head\_sam} & \textbf{2.7558} & 0.998 & yes \\
rule\_sam & 2.8517 & 0.998 & yes \\
two\_head & 2.8682 & 0.971 & yes \\
rule & 2.9563 & 0.970 & yes \\
commensurate\_like & 2.9688 & 0.968 & yes \\
weak\_only & 3.0939 & 0.998 & yes \\
map\_like & 3.1562 & 0.941 & yes \\
power\_prior\_like & 3.1601 & 0.941 & yes \\
\bottomrule
\end{tabular}
\end{table}
...
evaluated priors, with mean held-out NLL $2.794$, PIT Kolmogorov--Smirnov
distance to uniformity $0.177$ (essentially tied with the two-head prior alone
at $0.175$), empirical 95\% credible-interval coverage
$0.996$, and reliability slope $1.36$; rule-SAM was similar (KS $0.243$, slope
$1.30$). These diagnostics are improved but not perfect: the reliability slope
...
$0.29$, KS $0.33$; 50\% intervals covered only $0.44$). Conflict adaptation
...
\midrule
\textbf{two\_head\_sam} & \textbf{2.794} & 0.177 & 0.622 & 0.915 & 0.996 & \textbf{1.36} \\
two\_head & 2.896 & \textbf{0.175} & 0.576 & 0.869 & 0.974 & 0.77 \\
rule\_sam & 2.901 & 0.243 & 0.532 & 0.856 & 0.995 & 1.30 \\
rule & 3.010 & 0.245 & 0.485 & 0.807 & 0.972 & 0.68 \\
weak\_only & 3.164 & 0.331 & 0.443 & 0.794 & 0.994 & -- \\
map\_like & 3.222 & 0.346 & 0.389 & 0.689 & 0.945 & 0.45 \\
power\_prior\_like & 3.227 & 0.338 & 0.399 & 0.680 & 0.942 & 0.42 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
  ...
  reliability slope rises from $0.77$ for two-head alone to $1.36$ with SAM
  (values above $1.0$ indicate remaining miscalibration) --- and two-head + SAM
  shows the most favorable diagnostics overall among the compared priors
  (PIT-KS $0.177$, essentially tied with two-head alone at $0.175$;
...
outcomes are cleaner) while preserving the method ordering and every
qualitative conclusion; the one ordering change is that the trained two-head
prior now narrowly outperforms the conflict-adapted rule baseline, where the
pre-correction order was reversed. Forward-validation improvements remained
stable with all paired bootstrap confidence intervals excluding zero. Three
things were never affected. The two worked case studies
...
are leakage-free as well, having been rescored with two-head DeepSets
models retrained on past-only data at each cutoff; these per-fold models were
trained on 632, 813, and 1{,}017 past-only examples; the training code now
