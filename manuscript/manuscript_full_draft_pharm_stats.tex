...
over five training seeds, and the pre-registered five-seed ensemble prior
exceeds the EB reference on the pooled forward windows ($2.8084$ vs
$2.8440$; $\Delta$ $-0.0356$, CI $[-0.0670,-0.0059]$), with the window-3
lead surviving Bonferroni adjustment. These
forward gains presuppose that donor results existed to be borrowed: under a
strict registry-availability replay (donor results posted before each query's
start date) most queries retain no usable donor --- rising from $2\%$
availability in the development era to $31\%$ in the latest window --- the
pooled advantage over EB disappears ($\Delta$ $-0.001$, CI
$[-0.017,+0.016]$), and the selective prior's contribution reduces to doing
no harm; registry availability is rising steeply with calendar time, and
registry posting understates what sponsors can read in the literature. The learned
...
robust-MAP; the anchored SAM adapter restored control
($0.070$ at the same shifts, $0.046$ at the strongest) while retaining the
no-conflict power advantage, so the outcome-adaptive layer is a required
companion of the selective prior, not an optional sensitivity analysis. Ablation showed that the prospective set-level conflict
...
controls almost entirely, matching the EB-only reference ($0.018$).

\noindent\textbf{Conclusions.} The framework provides a reproducible
methodology prototype and an EB-referenced, leakage-partitioned evaluation
protocol for oncology historical borrowing. Its defensible contributions are a
...
borrowing and yielding an interpretable design-time triage policy---and the
finding, of independent cautionary value, that against a properly fitted
marginal EB prior the average real-data benefit of candidate-level borrowing
is modest and concentrated where retrieved candidate sets are
information-rich; known-truth simulation shows when borrowing helps and when
it harms. These analyses do not establish average superiority over marginal
...
\emph{observation-process} rate $\theta^{\mathrm{obs}}_k = \theta_k +
\mathrm{bias}_k$, where $\mathrm{bias}_k$ models endpoint-definition and
reporting distortion; the donor's data are then drawn
...
free of outcome and fitting leakage by construction; what it does not yet
enforce is donor-level availability at decision time (a donor completed before
...
the partition rule of Section~\ref{sec:eval}.
...
a step pre-registered, before any of it was run, as the one remaining
mechanical lever, to be reported whichever way it came out. The architectural
...
single pre-specified aggregate test, now excluding zero), $-0.0755$
...
single-seed analysis as primary because it was specified first, and the
ensemble as the pre-registered upgrade; both are subject to the
availability scoping below. Marginal
...
survives multiplicity adjustment under the ensemble), the pre-registered
...
and none of it survives the strict registry-availability replay below.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{F2_forward_validation}
  \caption{\textbf{Forward validation on disjoint future
  windows.} (a)~NLL pooled over the three disjoint windows ($n=775$) for
...
\begin{table}[htbp]
\centering
\caption{Forward validation on disjoint future windows (outcome-fit temporal split with past-only refits; donor availability at decision time not enforced,
design-time priors only). NLL pooled over the three windows ($n=775$); paired
delta versus the past-fit intercept-EB reference with bootstrap $95\%$ CI
where the comparison is primary. Learned rows and EB anchors are refit on
past-only data at each window's left cutoff.}
\label{tab:forward}
\begin{tabular}{lrl}
\toprule
...
\paragraph{Donor availability at decision time.} The analysis above controls
outcome and fitting leakage but not whether a donor's \emph{results} had been
posted when they would have been needed, so we repeated it under two
availability rules using the results-first-posted date of every trial in the
corpus. Availability is scarce and strongly time-graded: under the strict
design-time rule (donor results posted before the query trial's start date),
only $4.2\%$ of gated candidate observations survive, and the fraction of
queries retaining any usable gated donor rises from $2\%$ of
development-era queries to $8.8\%$, $20.1\%$, and $31.3\%$ across the three
forward windows --- and from $0\%$ of pre-2015 starts to $36\%$ of 2023
starts --- reflecting the maturation of mandatory results posting. Under the
analysis-time rule (everything an analyst could have used at each cutoff:
training queries and donors restricted to results posted by the cutoff), the
training pools shrink from $632/813/1{,}017$ to $323/494/651$ queries and the
surviving donor observations to $27\%/36\%/45\%$; the EB anchors are almost
unchanged ($\mathrm{Beta}(0.68,1.66)$ at the final cutoff). Retraining the
selective model in this world and rescoring every forward query with the
strict per-query donor rule leaves the scored sets unchanged but removes most
borrowing opportunity, and the result is neutrality: pooled $\Delta$ versus
the availability-consistent EB reference $-0.001$ (CI $[-0.017,+0.016]$), and
$-0.003$ (CI $[-0.072,+0.072]$) within the $179$ queries that retained an
available gated donor. Two readings follow, and we state both. The
architectural comparisons of Table~\ref{tab:forward} measure what borrowing
of registry-posted results \emph{could} contribute where such results exist
--- an increasingly realistic condition, and one that also understates
availability insofar as sponsors read publications years before registry
posting --- while under a strict registry-only replay of each query's design
date, most of the aggregate advantage over EB is not yet realisable, and the
selective prior's virtue reduces to doing no harm. Claims elsewhere in the
paper are scoped accordingly.
...
$-0.018$ and $-0.037$. (Connected components of the sharing graph are
degenerate here --- one component holds $98\%$ of queries --- so we report
the finer block scheme.) Separately, because $54\%$ of donor observations and
...
Third, the anchored SAM adapter restores control while keeping the
...
variant --- with power $0.594$ at zero conflict, still above every
...
that design-time features cannot see.
...
\hline
 & \multicolumn{4}{c}{Type I error} & \multicolumn{3}{c}{Power} \\
Method & $0.0$ & $0.5$ & $1.0$ & $1.5$ & $0.0$ & $0.5$ & $1.0$ \\
\hline
No borrowing                    & 0.037 & 0.029 & 0.034 & 0.028 & 0.536 & 0.544 & 0.561 \\
EB-only reference               & 0.030 & 0.022 & 0.032 & 0.022 & 0.497 & 0.514 & 0.526 \\
Rule mixture                    & 0.029 & 0.030 & 0.050 & 0.049 & 0.556 & 0.609 & 0.635 \\
Rule + SAM                      & 0.030 & 0.030 & 0.048 & 0.042 & 0.555 & 0.600 & 0.603 \\
Two-head + prospective          & 0.025 & 0.039 & 0.072 & 0.061 & 0.588 & 0.628 & 0.627 \\
Two-head + pro.\ + SAM          & 0.029 & 0.040 & 0.070 & 0.051 & 0.588 & 0.622 & 0.600 \\
Two-head + pro., fixed disc.    & 0.027 & 0.032 & 0.056 & 0.057 & 0.573 & 0.620 & 0.643 \\
Selective (prospective)         & 0.024 & 0.056 & 0.145 & 0.243 & 0.613 & 0.700 & 0.766 \\
Selective (no prospective)      & 0.026 & 0.046 & 0.110 & 0.185 & 0.599 & 0.685 & 0.752 \\
Selective, fixed discount       & 0.025 & 0.035 & 0.079 & 0.108 & 0.586 & 0.656 & 0.734 \\
Selective + SAM (anchored)      & 0.029 & 0.041 & 0.070 & 0.046 & 0.594 & 0.616 & 0.582 \\
Robust-MAP ($w=0.5$)            & 0.037 & 0.035 & 0.047 & 0.043 & 0.575 & 0.603 & 0.609 \\
UIP-JS (uses outcome)           & 0.050 & 0.048 & 0.069 & 0.081 & 0.625 & 0.670 & 0.721 \\
Power prior ($a_0=0.3$)         & 0.484 & 0.701 & 0.863 & 0.936 & 0.861 & 0.917 & 0.970 \\
Full pooling                    & 0.584 & 0.777 & 0.904 & 0.959 & 0.880 & 0.930 & 0.974 \\
\hline
\end{tabular}
\end{table}
...
posted are different populations}: under a strict registry-availability
replay of each query's design date, most historical results were not yet
posted, the pooled advantage over EB disappears, and the framework's
...
the anchored SAM companion --- while in the external-control world, trained
on a conflict-rich draw, the same head learns to withhold external controls
almost entirely. These simulation analyses do not
...
\paragraph{Expert manual audit.} An expert audit of $80$ extracted
...
the ten most-shared donors, each checked against the live registry on six
items --- completes the extraction-quality picture, and its two halves point
in opposite directions. \emph{Numeric} integrity is high: arm identification
$98.8\%$, numerator $97.5\%$, denominator $97.5\%$, analysis population
$97.5\%$; all four simultaneously, $93.8\%$ ($95\%$ CI $86.2$--$97.3\%$). The
...
disease-appropriate equivalent) was $77.5\%$ ($62/80$; CI $67.2$--$85.3\%$),
...
prior rather than an optional sensitivity analysis. Third, the availability-controlled analysis of
Section~\ref{sec:forward-results} bounds what the forward validation can
claim: under a strict registry-only replay of each query's design date, most
of the pooled advantage over the EB reference is not realisable, because the
donor results had not yet been posted. Two residual gaps remain even in that
analysis. The stored registry records are current versions rather than
as-of-cutoff snapshots, so endpoint text edited after completion could differ
from what a contemporaneous user saw (the query side is protected --- the
entire results section of each pseudo-query is removed before retrieval ---
but full snapshot reconstruction remains future work); and registry posting
understates true availability, since results are typically published in the
literature before they are posted, so the strict replay is a lower bound just
as the unfiltered analysis is an upper bound. Fourth, marginal
...
them), and the stratified expert audit of Section~\ref{sec:limitations-data}
...
conclusion is precise. Selective borrowing improves substantially on the
flawed fixed-budget predecessor --- in and out of time, under query-level and
donor-block resampling alike, and under every training seed. Against
appropriately fitted marginal empirical-Bayes references, single-fit
selective priors are consistently but not significantly ahead; the
pre-registered five-seed ensemble does exceed the intercept-EB reference on
the pooled forward windows ($\Delta$ $-0.0356$, CI $[-0.0670,-0.0059]$),
an advantage attributable to the interaction of the EB anchor with learnable
total borrowing and concentrated where candidate information is rich; and
its demonstrated per-query value is triage, above all the
elimination of forced-borrowing harm where the candidate set carries little
usable information. Under a strict registry-availability replay of each
query's design date, most of the remaining aggregate advantage is not yet
realisable at all, because the donor results had not been posted --- a
constraint that is relaxing quickly as registry reporting matures. The
...
conflict (type I error up to $0.243$ standalone, at a nominal $0.025$), so
the outcome-adaptive SAM companion --- which restores control ($\le 0.070$)
while keeping the no-conflict advantage --- is a required part of the
proposal, not a sensitivity analysis.
...
availability at decision time not yet enforced. & See
Sections~\ref{sec:eval} and 6.2. \\
