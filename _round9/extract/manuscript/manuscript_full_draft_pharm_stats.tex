...
uncertainty is assessed by repeated random splitting (mean $-0.0007$,
median $-0.0009$; central $95\%$ split-quantile range
$[-0.0014,+0.0039]$). A cell-specific ROC-efficiency
...
oncology data --- nothing yet on decision-time-available data, no
established mixture-average gain in simulation (an observed cell-specific
ROC-efficiency advantage at small drift, with the size-adjusted average
not established in either direction), and an upper-bound gain
...
$200$ random half-splits --- so that sensitivity to the split and to
threshold estimation is assessed rather than hidden --- centres the final
...
threshold, not a cell-specific power structure. The
...
result above, the honest summary is: \emph{all non-repeated plug-in
estimates were positive, and the sign is not established once
threshold-estimation
...
dispersion is wide) is robust --- robust now also under repetition:
repeating the cross-fit on the identical $200$ splits under narrower
mixings gives a positive size-adjusted difference in \emph{every} split
($N(0,1)$: mean $+0.0097$, central $95\%$ split-quantile range
$[+0.0090,+0.0149]$; $N(0,0.5)$: mean $+0.0382$, range
$[+0.0365,+0.0432]$), against $-0.0007$ under the corpus mixing. What
the analysis supports,
exploratorily, is that the final design gives up essentially nothing on
average under this corpus's drift profile, and that what it exhibits
beyond the average is an observed local ROC-efficiency advantage, not an
established average one. The frozen-threshold profile differences
...
observed in this corpus, size-adjusted}, all non-repeated plug-in
...
$-0.0007$, central $95\%$ split-quantile range $[-0.0014,+0.0039]$), with only the
...
estimated on the validation batch itself, gave positive non-repeated
plug-in estimates
throughout ($+0.002$ to $+0.006$), but repeated random splitting centres
the size-adjusted difference near zero (mean $-0.0007$, median
$-0.0009$; central $95\%$ split-quantile range
$[-0.0014, +0.0039]$, a descriptive range conditional on the validation
batch, not a confidence interval) --- no average net power difference at matched
